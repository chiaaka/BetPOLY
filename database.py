...
async def _migrate():
    ...
    cols = [
        ...
        ("users", "total_wins", "ALTER TABLE users ADD COLUMN total_wins INTEGER DEFAULT 0"),
        ("users", "total_losses", "ALTER TABLE users ADD COLUMN total_losses INTEGER DEFAULT 0"),
        ("users", "total_profit", "ALTER TABLE users ADD COLUMN total_profit REAL DEFAULT 0"),
    ]
    async with aiosqlite.connect(_DB) as db:
        for _, _, sql in cols:
            try:
...
async def get_user_stats(telegram_id: int) -> dict:
    ...
    async with aiosqlite.connect(_DB) as conn:
        conn.row_factory = aiosqlite.Row
        row = await conn.execute_fetchall(
            "SELECT total_volume, total_wins, total_losses, total_profit, total_fees_paid FROM users WHERE telegram_id = ?",
            (telegram_id,)
        )
        if row:
            r = row[0]
            return {
                "volume": float(r[0] or 0),
                "wins": int(r[1] or 0),
                "losses": int(r[2] or 0),
...
async def record_win(telegram_id: int, profit: float):
    ...
    async with aiosqlite.connect(_DB) as conn:
        await conn.execute(
            "UPDATE users SET total_wins = total_wins + 1, total_profit = total_profit + ? WHERE telegram_id = ?",
            (profit, telegram_id)
        )
        await conn.commit()
...
async def record_loss(telegram_id: int, loss_amount: float):
    ...
    async with aiosqlite.connect(_DB) as conn:
        await conn.execute(
            "UPDATE users SET total_losses = total_losses + 1, total_profit = total_profit - ? WHERE telegram_id = ?",
...
async def was_notified(telegram_id: int, token_id: str) -> bool:
    ...
    async with aiosqlite.connect(_DB) as conn:
        row = await conn.execute_fetchall(
            "SELECT id FROM settlement_notifications WHERE telegram_id = ? AND token_id = ?",
...
async def mark_notified(telegram_id: int, token_id: str, status: str):
    ...
    async with aiosqlite.connect(_DB) as conn:
        await conn.execute(
            "INSERT INTO settlement_notifications (telegram_id, token_id, status, created_at) VALUES (?, ?, ?, ?)",
...
async def was_redeemed(telegram_id: int, token_id: str) -> bool:
    ...
    async with aiosqlite.connect(_DB) as conn:
        row = await conn.execute_fetchall(
            "SELECT id FROM settlement_notifications WHERE telegram_id = ? AND token_id = ? AND status = 'redeemed'",
            (telegram_id, token_id[:40])
        )
        return bool(row)
