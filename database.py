...
async def init_db():
    async with aiosqlite.connect(_DB) as db:
        ...
        # Track alerts sent for position changes (won/lost/redeemable)
        await db.execute("""
            CREATE TABLE IF NOT EXISTS position_alerts (
                ...
                alert_type TEXT,
                created_at REAL,
                UNIQUE(telegram_id, asset, alert_type)
            )
        """)
...
async def get_bet_by_id(bet_id: int) -> dict:
    async with aiosqlite.connect(_DB) as db:
        db.row_factory = aiosqlite.Row
...
async def has_alert_been_sent(telegram_id: int, asset: str, alert_type: str) -> bool:
    """Check if we already sent this alert."""
    async with aiosqlite.connect(_DB) as db:
        cur = await db.execute(
            "SELECT id FROM position_alerts WHERE telegram_id=? AND asset=? AND alert_type=?",
            (telegram_id, asset, alert_type))
        return (await cur.fetchone()) is not None


async def mark_alert_sent(telegram_id: int, asset: str, alert_type: str):
    """Record that we sent this alert (won't duplicate due to UNIQUE constraint)."""
    async with aiosqlite.connect(_DB) as db:
        try:
            await db.execute(
                "INSERT OR IGNORE INTO position_alerts (telegram_id, asset, alert_type, created_at) VALUES (?,?,?,?)",
                (telegram_id, asset, alert_type, time.time()))
            await db.commit()
        except:
            pass
...
async def get_all_users_with_wallets() -> list:
    """Get all users who have a wallet address (for background monitoring)."""
    async with aiosqlite.connect(_DB) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            "SELECT telegram_id, wallet_address, currency FROM users WHERE wallet_address IS NOT NULL AND wallet_address != ''")
        return [dict(r) for r in await cur.fetchall()]
