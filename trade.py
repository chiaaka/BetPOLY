...
async def place_bet(
    ...
    try:
        ...
        try:
            ...
            # Check if USDC.e allowances are 0
            allowances = bal.get("allowances", {})
            usdc_zero = all(int(v) == 0 for v in allowances.values()) if allowances else True
            
            # Check if CTF allowances are 0  
            cond_allowances = cond_bal.get("allowances", {})
            ctf_zero = all(int(v) == 0 for v in cond_allowances.values()) if cond_allowances else True
            
            if usdc_zero or ctf_zero:
                needs_approve = True
                logger.info(f"  ⚠️ Missing allowances (USDC={usdc_zero}, CTF={ctf_zero}), running approve...")
