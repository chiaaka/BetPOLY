...
async def place_bet(
    ...
    try:
        ...
        if shares < min_size:
            shares = min_size
            # Recalculate cost with minimum shares
            clob_cost_micro = math.floor(shares * calc_price * 1_000_000)
            order_cost = clob_cost_micro / 1_000_000
            total_needed = order_cost + fee
            
            if balance < total_needed:
                result["error"] = f"Minimum bet for this market is {min_size} shares (${order_cost:.2f}). You have ${balance:.2f}."
                return result
            
            logger.info(f"  Adjusted to minimum: shares={shares}, cost=${order_cost}")
