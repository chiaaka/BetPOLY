...
async def place_bet(
    ...
    try:
        ...
        # Use FOK market order (like Polymarket website) to bypass min_order_size
        # Market orders specify dollar amount, not shares — no minimum share count
        ...
        logger.info(f"  Posting FOK market order: amount=${order_cost} price={price} shares~{shares} neg_risk={neg_risk} tick_size={tick_size}")
        
        try:
            # Primary: FOK market order (same as Polymarket website)
            ...
            # Try create_market_order with options (neg_risk needed for correct signing)
            try:
                signed_order = client.create_market_order(market_order_args, options=options)
            except TypeError:
                # Older py-clob-client may not accept options kwarg
                signed_order = client.create_market_order(market_order_args)
            response = client.post_order(signed_order, OrderType.FOK)
        except Exception as fok_err:
            fok_str = str(fok_err)
            logger.warning(f"  FOK market order failed: {fok_str[:150]}")
