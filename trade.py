    ...
    try:
        ...
        try:
            ...
            if isinstance(cond_bal, dict):
                ...
                # Conditional tokens use 6 decimals (like USDC)
                actual_shares = raw_balance / 1_000_000 if raw_balance > 1000 else raw_balance / 100
                logger.info(f"  Conditional balance: raw={raw_balance} (~{actual_shares:.4f} shares, want to sell {sell_shares})")
                
                # If we're trying to sell more than we have, adjust
                if actual_shares > 0 and actual_shares < sell_shares:
                    logger.warning(f"  Adjusting sell amount from {sell_shares} to {actual_shares:.2f}")
                    sell_shares = round(actual_shares, 2)
                elif raw_balance == 0:
...
async def collect_fee(private_key: str, safe_address: str, fee_usdc: float) -> dict:
    ...
    try:
        ...
        tx = {"to": USDC_E, "data": transfer_data, "value": "0"}
