...
async def place_bet(
    ...
    try:
        ...
        # CRITICAL: Tell CLOB to refresh its cached view of our on-chain balance/allowance
        # Without this, the CLOB may not see our USDC.e or approvals
        try:
            from py_clob_client.clob_types import BalanceAllowanceParams, AssetType
            # Update USDC.e (collateral) balance cache
            client.update_balance_allowance(BalanceAllowanceParams(
                asset_type=AssetType.COLLATERAL
            ))
            logger.info("  ✅ CLOB balance cache refreshed (COLLATERAL)")
            
            # Also get balance to log what CLOB sees
            bal = client.get_balance_allowance(BalanceAllowanceParams(
                asset_type=AssetType.COLLATERAL
            ))
            logger.info(f"  CLOB sees: {bal}")
        except Exception as ube:
            logger.warning(f"  Could not refresh CLOB balance cache: {ube}")
