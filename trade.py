...
def set_allowances(private_key: str) -> bool:
    ...
    Tries gasless via relayer first, falls back to on-chain (pays gas).
    """
    # Try gasless via relayer first (requires Builder credentials)
    try:
        import wallet as w_mod
        if w_mod.set_approvals(private_key):
            logger.info("  Approvals set via relayer (gasless)")
            return True
    except Exception as e:
        logger.warning(f"  Gasless approval failed ({e}), trying on-chain...")
    
    # Fallback: on-chain approvals (requires POL for gas)
...
def _get_clob_client(private_key: str, funder_address: str = None) -> ClobClient:
    ...
    If funder_address is provided (Safe wallet pattern):
      signature_type=2 (EOA associated to Gnosis Safe)
      funder=safe_address
    Otherwise (legacy EOA):
    ...
    sig_label = f"2/Safe({funder_address[:10]})" if funder_address else "0/EOA"
    logger.info(f"  CLOB client ready ({sig_label}), API key: {api_creds.api_key[:8]}...")
    return client
...
async def place_bet(
    ...
    Args:
        private_key: User's wallet private key (EOA signer)
        ...
        funder_address: Safe wallet address (if using Safe pattern)
    ...
    try:
        ...
        # Check balance (on the Safe address if using Safe pattern)
        balance = get_usdc_balance(address=funder_address) if funder_address else get_usdc_balance(private_key)
        ...
        if needs_approve:
            # Try gasless first (Safe pattern), then fall back to on-chain
        ...
        check_addr = funder_address or ""
        if check_addr:
            bals = get_usdc_balances(address=check_addr)
            logger.info(f"  On-chain (Safe): USDC.e=${bals['usdc_e']}, Native USDC=${bals['usdc_native']}")
        else:
            bals = get_usdc_balances(private_key=private_key)
            logger.info(f"  On-chain (EOA): USDC.e=${bals['usdc_e']}, Native USDC=${bals['usdc_native']}, POL={get_matic_balance(private_key=private_key)}")
...
async def sell_position(private_key: str, funder_address: str,
    ...
    try:
        ...
        # Pre-fetch price for proceeds estimation (non-blocking, best effort)
        sell_price = None
        try:
            price_data = client.get_price(token_id, side="SELL")
            if price_data:
                sell_price = float(price_data)
                logger.info(f"  Pre-fetch sell price: {sell_price}")
        except:
            pass
        ...
        if response and response.get("orderID"):
            estimated_proceeds = round(shares * sell_price, 4) if sell_price else 0
            result["success"] = True
            result["order_id"] = response["orderID"]
...
async def redeem_position(private_key: str, condition_id: str, neg_risk: bool = False) -> dict:
    """
    Redeem won positions on-chain via the CTF contract.
    Burns winning outcome tokens and returns collateral.
    
    For standard markets: call CTF.redeemPositions with USDC.e as collateral
    For neg_risk markets: call NegRiskAdapter.redeemPositions(conditionId, amounts[])
        - The adapter wraps the CTF call and handles WrappedCollateral conversion
    
    Returns {"success": True, "tx_hash": str} or {"success": False, "error": str}
    """
    ...
    try:
        ...
        # Multiple RPC endpoints for reliability (free public ones rate limit)
        RPC_URLS = [
        ...
        NEG_RISK_EXCHANGE = w3.to_checksum_address("0xC5d563A36AE78145C45a50134d48A1215220f80a")
        ...
        logger.info(f"  Redeem: condition={condition_id[:16]}... neg_risk={neg_risk} wallet={wallet_addr[:10]}...")
        
        # Get nonce
        nonce = w3.eth.get_transaction_count(wallet_addr)
        gas_price = w3.eth.gas_price
        
        # ERC1155 setApprovalForAll ABI (needed to allow CTF to move our tokens)
        approval_abi = [
            {
                "inputs": [
        ...
        ctf_for_approval = w3.eth.contract(address=CTF, abi=approval_abi)
        
        if neg_risk:
            # --- NEG RISK REDEEM ---
            # Step 1: Ensure CTF is approved for NegRiskAdapter to move tokens
            is_approved = ctf_for_approval.functions.isApprovedForAll(
                wallet_addr, NEG_RISK_ADAPTER
            ).call()
            
            if not is_approved:
                logger.info(f"  Setting CTF approval for NegRiskAdapter...")
                approve_txn = ctf_for_approval.functions.setApprovalForAll(
                    NEG_RISK_ADAPTER, True
                ).build_transaction({
                    'from': wallet_addr,
                    'nonce': nonce,
                    'gas': 100000,
                ...
                nonce += 1
                logger.info(f"  CTF approval set for NegRiskAdapter")
            
            # Step 2: Call NegRiskAdapter.redeemPositions(conditionId, amounts[])
            neg_risk_abi = [
                {
                    "inputs": [
            ...
            txn = adapter.functions.redeemPositions(
                condition_id_bytes,
                []  # Empty amounts = redeem all available
            ).build_transaction({
                'from': wallet_addr,
                'nonce': nonce,
                'gas': 400000,
            ...
            # --- STANDARD CTF REDEEM ---
            # Step 1: Ensure CTF approval for itself (sometimes needed)
            is_approved = ctf_for_approval.functions.isApprovedForAll(
                wallet_addr, CTF
            ).call()
            
            if not is_approved:
                logger.info(f"  Setting CTF self-approval...")
                approve_txn = ctf_for_approval.functions.setApprovalForAll(
                    CTF, True
                ).build_transaction({
                    'from': wallet_addr,
                ...
                w3.eth.wait_for_transaction_receipt(approve_hash, timeout=30)
                nonce += 1
                logger.info(f"  CTF self-approval set")
            
            # Step 2: Call CTF.redeemPositions directly
            ctf_redeem_abi = [
                {
                    "inputs": [
            ...
            ctf_contract = w3.eth.contract(address=CTF, abi=ctf_redeem_abi)
        ...
        logger.info(f"  Redeem tx sent: {tx_hex}")
        ...
        if receipt.status == 1:
            ...
            logger.info(f"  ✅ Redeem confirmed! tx: {tx_hex}")
        else:
            # Try to get revert reason
            result["error"] = "Transaction reverted on-chain. The market may not be fully resolved yet."
            logger.error(f"  ❌ Redeem reverted: {tx_hex}")
    
    except Exception as e:
        error_str = str(e)
        logger.error(f"  ❌ Redeem error: {error_str[:200]}")
        
        if "insufficient funds" in error_str.lower():
            result["error"] = "Not enough MATIC/POL for gas fees."
        elif "execution reverted" in error_str.lower():
            result["error"] = "Transaction reverted. Market may not be fully resolved yet, or tokens not in your wallet."
        else:
            result["error"] = f"Redeem failed: {error_str[:100]}"
