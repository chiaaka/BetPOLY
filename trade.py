...
def set_allowances(private_key: str) -> bool:
    ...
    try:
        ...
        # 1. Approve USDC.e for CTF Exchange + Neg Risk CTF Exchange
...
async def place_bet(
    ...
    try:
        ...
        # Must round to 2 decimal places (Polymarket requirement)
        # Round UP by 0.01 shares to ensure size*price >= $1 after CLOB rounding
        import math
        shares_exact = net_amount / price
        shares = math.ceil(shares_exact * 100) / 100  # Round up to 2 decimals
        
        # If rounding still doesn't meet $1 minimum (floating point), add 0.01
        if shares * price < 1.0:
            shares = round(shares + 0.01, 2)
        
        # Actual cost to user (may be slightly more than net_amount due to rounding)
        order_cost = round(shares * price, 4)
        total_needed = order_cost + fee  # Actual amount needed from wallet
        ...
        logger.info(f"Placing bet: {match_name} | {selection} | "
                    ...
                    f"price={price} shares={shares} cost=${order_cost} token={token_id[:12]}...")
        ...
        try:
            ...
            logger.info(f"  CLOB sees: {bal}")
            
            # Check if allowances are actually 0 — need to approve
            allowances = bal.get("allowances", {})
            all_zero = all(int(v) == 0 for v in allowances.values()) if allowances else True
            
            if all_zero:
                logger.info("  ⚠️ CLOB reports allowances=0, running approve...")
                matic_bal = get_matic_balance(private_key=private_key)
                if matic_bal < 0.01:
                    result["error"] = f"Need ~0.02 POL for gas to approve contracts (one-time setup). You have {matic_bal:.4f} POL. Please deposit more POL."
                    return result
                ...
                bal2 = client.get_balance_allowance(BalanceAllowanceParams(
                    asset_type=AssetType.COLLATERAL
                ))
                logger.info(f"  CLOB after approve: {bal2}")
        ...
        # Also try to get minimum_order_size from market data
        try:
            market_info = client.get_market(token_id)
            if market_info and isinstance(market_info, dict):
                min_size = float(market_info.get("minimum_order_size", 1.0))
                logger.info(f"  Minimum order size: {min_size} shares")
        except Exception as mse:
            logger.warning(f"  Could not get min order size: {mse}")
        
        # Ensure shares meet minimum order size
        if shares < min_size:
            # Need more USDC to meet minimum
            needed = round(min_size * price, 2)
            result["error"] = f"Minimum order is {min_size} shares (${needed:.2f}). Please increase your bet."
            return result
        ...
        # Create and post order with explicit neg_risk
        ...
        logger.info(f"  Posting order: shares={shares} price={price} neg_risk={neg_risk} tick_size={tick_size}")
