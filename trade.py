...
async def sell_position(private_key: str, funder_address: str,
    ...
    try:
        ...
        if not sell_price or sell_price <= 0:
            result["error"] = "No buyers available for this position right now."
            return result
        ...
        logger.info(f"  Selling: {shares} shares of {token_id[:20]}... @ {sell_price} (neg_risk={neg_risk})")
        ...
        try:
            ...
            response = client.post_order(signed_order, OrderType.FOK)
            logger.info(f"  Sell Strategy 1 (MarketOrderArgs) succeeded")
        except Exception as e1:
            logger.warning(f"  Sell Strategy 1 failed: {str(e1)[:100]}")
            
            # Strategy 2: FOK limit sell
            try:
                order_args = OrderArgs(
                    token_id=token_id,
                    price=sell_price,
                ...
                signed_order = client.create_order(order_args, options=options)
                response = client.post_order(signed_order, OrderType.FOK)
                logger.info(f"  Sell Strategy 2 (FOK limit) succeeded")
            except Exception as e2:
                logger.warning(f"  Sell Strategy 2 failed: {str(e2)[:100]}")
                
                # Strategy 3: GTC limit sell
                order_args = OrderArgs(
                    token_id=token_id,
                    price=sell_price,
                    size=shares,
                ...
                response = client.create_and_post_order(order_args, options=options)
                logger.info(f"  Sell Strategy 3 (GTC limit) succeeded")
        
        if response and response.get("orderID"):
            estimated_proceeds = round(shares * sell_price, 4)
            result["success"] = True
            result["order_id"] = response["orderID"]
