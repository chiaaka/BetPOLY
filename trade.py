...
async def get_positions(private_key: str, safe_address: str = "") -> list:
    ...
    try:
        ...
        # Get already-redeemed tokens from activity API
        redeemed_tokens = set()
        redeemed_conditions = set()
        try:
            redeem_resp = requests.get(
                "https://data-api.polymarket.com/activity",
                params={"user": safe_address, "type": "REDEEM", "limit": 500},
                timeout=10,
            )
            if redeem_resp.ok:
                redeem_data = redeem_resp.json() or []
                logger.info(f"  Found {len(redeem_data)} REDEEM activities")
                for a in redeem_data:
                    asset = a.get("asset", "")
                    cond = a.get("conditionId", "")
                    if asset:
                        redeemed_tokens.add(asset)
                    if cond:
                        redeemed_conditions.add(cond)
        except Exception as e:
            logger.debug(f"  Redeem activity fetch failed: {e}")
        ...
        for p in data:
            ...
            if redeemable:
                if token_id in redeemed_tokens or condition_id in redeemed_conditions:
                    logger.info(f"  Skipping redeemed position: {token_id[:15]}...")
                    continue  # Skip entirely — don't even add to list
                else:
                    status = "won"
            elif size < 0.01:
