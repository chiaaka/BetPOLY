...
async def place_bet(
    ...
    try:
        ...
        if needs_approve:
            # Try to set approvals via wallet.py (Safe-based)
            try:
                import wallet as _wallet
                matic_bal = get_matic_balance(private_key=private_key)
                if matic_bal < 0.01:
                    result["error"] = f"Need ~0.1 POL for one-time wallet setup. You have {matic_bal:.4f} POL. Send POL to your EOA address."
                    return result
