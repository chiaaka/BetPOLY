...
async def place_bet(
    ...
    try:
        ...
        # Auto-approve on first trade (one-time, checks before doing)
        if not check_allowances(private_key=private_key):
            logger.info("  Allowances not set — running auto-approve...")
            matic_bal = get_matic_balance(private_key=private_key)
            if matic_bal < 0.005:
                result["error"] = "Need ~0.01 POL for gas to approve contracts (first-time setup). Please deposit some POL/MATIC."
                return result
            approved = set_allowances(private_key)
            if not approved:
                result["error"] = "Failed to set contract approvals. Please try again."
                return result
        
        # Debug: Check what CLOB sees as our balance
        try:
            bal_info = client.get_balance_allowance(token_id=token_id)
            logger.info(f"  CLOB balance/allowance: {bal_info}")
        except Exception as be:
            logger.warning(f"  Could not query CLOB balance: {be}")
