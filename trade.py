...
async def redeem_position(private_key: str, condition_id: str, neg_risk: bool = False) -> dict:
    ...
    Burns winning outcome tokens and returns USDC.e collateral.
    
    For EOA wallets (BetPoly's wallet type), we call redeemPositions directly.
    For neg_risk markets, we call through the NegRiskAdapter instead.
    ...
    try:
        ...
        # For neg_risk, the adapter has its own redeem
        neg_risk_redeem_abi = [
            {
                "constant": False,
                "inputs": [
        ...
        logger.info(f"  Redeem: condition={condition_id[:16]}... neg_risk={neg_risk} wallet={wallet[:10]}...")
        
        if neg_risk:
            # Neg risk markets: call NegRiskAdapter.redeemPositions
            # Pass empty amounts array — the adapter handles it
            contract = w3.eth.contract(address=NEG_RISK_ADAPTER, abi=neg_risk_redeem_abi)
            ...
            # Standard CTF: call redeemPositions directly
            contract = w3.eth.contract(address=CTF_ADDRESS, abi=redeem_abi)
            txn = contract.functions.redeemPositions(
                USDC_E,              # collateral token
                HASH_ZERO,           # parentCollectionId (always 0)
                condition_id_bytes,  # conditionId
                [1, 2]               # indexSets: [Yes, No] — redeem both slots
            ).build_transaction({
                'from': wallet,
                'nonce': w3.eth.get_transaction_count(wallet),
                'gas': 300000,
