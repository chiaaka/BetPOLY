...
async def get_positions(private_key: str, safe_address: str = "") -> list:
    ...
    try:
        ...
        # Set up on-chain CTF balance check (for redeemable positions)
        ctf_contract = None
        try:
            from web3 import Web3
            ...
            logger.debug(f"  CTF contract setup failed: {e}")
        ...
        for p in data:
            ...
            if redeemable:
                # On-chain check: if CTF balance is 0, already redeemed
                if ctf_contract and token_id:
                    try:
                        onchain_bal = ctf_contract.functions.balanceOf(
                            safe_checksum, int(token_id)
                        ).call()
                        if onchain_bal == 0:
                            logger.info(f"  Skipping redeemed position (on-chain bal=0): {token_id[:15]}...")
                            continue
                    except Exception as e:
                        logger.debug(f"  CTF balance check failed for {token_id[:15]}...: {e}")
                status = "won"
            elif size < 0.01:
