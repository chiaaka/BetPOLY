...
async def sell_position(
    ...
    """
    Sell a position — exactly like Polymarket.
    Uses FOK market order with side=SELL, amount=shares.
    
    Polymarket docs: "A FOK order to sell (in shares) must be executed 
    immediately in its entirety; otherwise, the entire order will be cancelled."
    
    For SELL: amount = number of shares (not dollars).
    """
    ...
    try:
        ...
        # First check conditional token balance
        try:
            from py_clob_client.clob_types import BalanceAllowanceParams, AssetType
            cond_bal = client.get_balance_allowance(BalanceAllowanceParams(
                asset_type=AssetType.CONDITIONAL,
            ...
            bal_amount = float(cond_bal.get("balance", 0)) if isinstance(cond_bal, dict) else 0
            logger.info(f"  Conditional token balance: {bal_amount} (need {shares})")
        except Exception as be:
            logger.warning(f"  Could not check conditional balance: {be}")
        
        # Update conditional token allowance if needed
        try:
            client.update_balance_allowance(BalanceAllowanceParams(
                asset_type=AssetType.CONDITIONAL,
                token_id=token_id,
        ...
        # Polymarket FOK SELL: amount = number of shares
        # Try with side=SELL first (v0.34.5+), fall back for v0.18.0
        try:
            market_order = MarketOrderArgs(
                token_id=token_id,
                amount=sell_shares,
            ...
            # v0.18.0 — no side param. Use OrderArgs limit order as fallback.
            logger.info("  Using limit order fallback for sell (v0.18.0)")
            
            # Get current bid price
            try:
                price_data = client.get_price(token_id, side="SELL")
                sell_price = float(price_data.get("price", 0.5)) if isinstance(price_data, dict) else float(price_data)
            except:
                sell_price = 0.5
            
            tick_decimals = len(tick_size.split(".")[-1]) if "." in tick_size else 2
            sell_price = round(sell_price * 0.98, tick_decimals)  # 2% slippage max
            ...
            logger.info(f"Selling (limit): token={token_id[:16]}... shares={sell_shares} price={sell_price}")
            response = client.create_and_post_order(order_args, options=options)
            
            if response and response.get("orderID"):
                result["success"] = True
                result["order_id"] = response["orderID"]
                result["amount"] = round(sell_shares * sell_price, 2)
        ...
        # FOK sell path (v0.34.5+)
        options = PartialCreateOrderOptions(neg_risk=neg_risk, tick_size=tick_size)
        
        logger.info(f"Selling (FOK): token={token_id[:16]}... shares={sell_shares}")
        signed = client.create_market_order(market_order, options=options)
        response = client.post_order(signed, OrderType.FOK)
...
async def collect_fee(private_key: str, safe_address: str, fee_usdc: float) -> dict:
    ...
    Uses ERC20 transfer via Builder Relayer (gasless for user).
    """
    ...
    try:
        ...
        # Encode: transfer(admin_wallet, fee_amount)
        transfer_data = (
            "0xa9059cbb"
        ...
        # Send via Builder Relayer (gasless)
        try:
            ...
            relayer = BuilderRelayerClient(signer, chain_id=137, creds=creds)
            ...
            tx_hash = relayer.execute(safe_address, [tx])
            result["success"] = True
            result["tx_hash"] = str(tx_hash)
            logger.info(f"  💸 Fee collected: ${fee_usdc:.4f} → {ADMIN_WALLET[:10]}... tx={tx_hash}")
            
        except ImportError:
            # Fallback: direct EOA transfer if no relayer
            logger.warning("Builder Relayer not available for fee collection")
            result["error"] = "Relayer not available"
