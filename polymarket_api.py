    ...
    for mkt in markets:
        ...
        # ==============================================
            ...
            if len(outcomes) >= 2 and _price_valid(prices[0] if prices else 0):
                # Build outcome->price/token mapping
                oc_map = []
                ...
                team_ocs = []
                for oc in oc_map:
                    if oc["name"].lower().strip() in ("draw", "tie"):
                        draw_oc = oc
                    else:
                        team_ocs.append(oc)
                ...
                # Match team outcomes to home/away by name
                home_lower = (home or "").lower()
                away_lower = (away or "").lower()
                
                matched_home = None
                matched_away = None
                
                for oc in team_ocs:
                    oc_lower = oc["name"].lower()
                    # Full name match
                    if home_lower and (home_lower in oc_lower or oc_lower in home_lower):
                        matched_home = oc
                    elif away_lower and (away_lower in oc_lower or oc_lower in away_lower):
                        matched_away = oc
                
                # If no full match, try unique word matching
                if not matched_home or not matched_away:
                    home_words = set(w for w in home_lower.split() if len(w) > 2) if home_lower else set()
                    away_words = set(w for w in away_lower.split() if len(w) > 2) if away_lower else set()
                    unique_home = home_words - away_words
                    unique_away = away_words - home_words
                    
                    for oc in team_ocs:
                        oc_lower = oc["name"].lower()
                        if not matched_home and unique_home and any(w in oc_lower for w in unique_home):
                            matched_home = oc
                        elif not matched_away and unique_away and any(w in oc_lower for w in unique_away):
                            matched_away = oc
                
                # Fallback: assign by position
                if not matched_home and not matched_away and len(team_ocs) >= 2:
                    matched_home = team_ocs[0]
                    matched_away = team_ocs[1]
                elif not matched_home and len(team_ocs) >= 1:
                    # Assign unmatched one
                    for oc in team_ocs:
                        if oc != matched_away:
                            matched_home = oc
                            break
                elif not matched_away and len(team_ocs) >= 1:
                    for oc in team_ocs:
                        if oc != matched_home:
                            matched_away = oc
                            break
                
                if matched_home and _price_valid(matched_home["price"]):
                    home_win_price = matched_home["price"]
                    home_win_tid = matched_home["tid"]
                    home_win_name = matched_home["name"]
                if matched_away and _price_valid(matched_away["price"]):
                    away_win_price = matched_away["price"]
                    away_win_tid = matched_away["tid"]
                    away_win_name = matched_away["name"]
                
                logger.info(f"  ML matched: home='{home_win_name}'@{home_win_price} away='{away_win_name}'@{away_win_price} draw={draw_price}")
        ...
        for mkt in markets:
            ...
            if len(outcomes) == 2 and outcome_names != ["yes", "no"]:
                if len(prices) >= 2 and (_price_valid(prices[0]) or _price_valid(prices[1])):
                    ...
                    oc0_name = str(outcomes[0])
                    oc1_name = str(outcomes[1]) if len(outcomes) > 1 else ""
                    oc0_price = prices[0]
                    oc1_price = prices[1] if len(prices) > 1 else 0
                    oc0_tid = tokens[0] if tokens else ""
                    oc1_tid = tokens[1] if len(tokens) > 1 else ""
                    
                    p_home = parsed.get("home", "").lower()
                    p_away = parsed.get("away", "").lower()
                    
                    # Check if outcome names match home/away
                    oc0_is_home = False
                    oc0_is_away = False
                    if p_home and (p_home in oc0_name.lower() or oc0_name.lower() in p_home):
                        oc0_is_home = True
                    elif p_away and (p_away in oc0_name.lower() or oc0_name.lower() in p_away):
                        oc0_is_away = True
                    
                    if not oc0_is_home and not oc0_is_away:
                        # Word matching
                        h_words = set(w for w in p_home.split() if len(w) > 2)
                        a_words = set(w for w in p_away.split() if len(w) > 2)
                        oc0_lower = oc0_name.lower()
                        if h_words and any(w in oc0_lower for w in h_words - a_words):
                            oc0_is_home = True
                        elif a_words and any(w in oc0_lower for w in a_words - h_words):
                            oc0_is_away = True
                    
                    if oc0_is_away:
                        # Swap: outcome 0 is actually away team
                        h_price, h_tid, h_name = oc1_price, oc1_tid, oc1_name
                        a_price, a_tid, a_name = oc0_price, oc0_tid, oc0_name
                    else:
                        # Default or confirmed: outcome 0 = home
                        h_price, h_tid, h_name = oc0_price, oc0_tid, oc0_name
                        a_price, a_tid, a_name = oc1_price, oc1_tid, oc1_name
                    
                    parsed["ml"] = {
                        "home": h_price, "home_tid": h_tid, "home_name": h_name,
                        "away": a_price, "away_tid": a_tid, "away_name": a_name,
                    ...
                    logger.info(f"  FALLBACK ML: home='{h_name}'@{h_price} away='{a_name}'@{a_price}")
                    break
...
async def verify_token_market(token_id: str) -> dict:
    ...
    try:
        ...
        logger.debug(f"Token verification lookup failed: {e}")
        return None
