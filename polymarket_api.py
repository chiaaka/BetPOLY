...
def format_match(g: dict, currency: str = "USD") -> str:
    ...
    if g["ml"]:
        ...
        # Lopsided market warning — only when one side is >85%
        if h_prob > 0 and a_prob > 0 and max(h_prob, a_prob) > 0.85:
            h_pct = f"{h_prob*100:.0f}%"
            a_pct = f"{a_prob*100:.0f}%"
            if labels["has_draw"] and ml.get("draw", 0) > 0:
                d_pct = f"{ml['draw']*100:.0f}%"
                lines.append(f"📊 {h_pct} · {d_pct} · {a_pct}")
            else:
                lines.append(f"📊 {h_pct} · {a_pct}")
            lines.append(f"⚠️ <i>Low liquidity — odds may not reflect actual fill price</i>")
