...
def format_match(g: dict, currency: str = "USD") -> str:
    ...
    # ── TOTALS / OVER-UNDER ──
    if g["totals"]:
        # Filter to only relevant game totals (removes quarter/half/player props)
        filtered_totals = _filter_totals_for_sport(g["totals"], sport)
        if not filtered_totals:
            filtered_totals = g["totals"][:3]  # fallback
    ...
    if g["spreads"]:
        ...
        for sp in filtered_spreads:
            ...
            if not a_line:
                # Invert: if home is -1.5, away is +1.5
...
def _pick_main_total(totals: list, sport: str) -> dict:
    ...
    # For basketball: pick highest line (game total, 200+)
    if sport in ("basketball", "american_football"):
        best = None
        for t in totals:
            try:
                line = float(t["line"])
                if sport == "basketball" and 190 <= line <= 280:
                    if best is None or line > float(best["line"]):
                        best = t
                elif sport == "american_football" and 25 <= line <= 75:
                    if best is None:
                        best = t
            except: pass
        ...
        # Prefer match total games over sets
        for t in totals:
            try:
                line = float(t["line"])
                if 15 <= line <= 45:
