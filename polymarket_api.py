...
def parse_event(event: dict) -> dict:
    """
    ⭐ FIXED: Correctly identifies Tennis Match Winner vs 1st Set Winner

    Uses sportsMarketType API field to distinguish:
    - "moneyline" = Match Winner (full match)
    - "tennis_first_set_winner" = 1st Set Winner only

    Returns BOTH in the parsed dict:
    - parsed["ml"] = Match Winner odds
    - parsed["set1"] = 1st Set Winner odds
    """
    ...
    logger.info(f"  parse_event: title={title[:40]} sport={sport}")

    parsed = {
        "title": title, "home": home, "away": away,
        "home_s": _abbreviate(home), "away_s": _abbreviate(away),
        "tournament": tournament,
        "set1": None,  # ⭐ 1ST SET WINNER NOW SET SEPARATELY
    ...
    # ════════════════════════════════════════════════════════════════
    # ⭐ PRE-LOOP: Use sportsMarketType to identify markets correctly
    # This is the ONLY reliable way to distinguish Tennis markets
    # ════════════════════════════════════════════════════════════════

    logger.info(f"  ┌─ TENNIS FIX: Scanning {len(markets)} markets by sportsMarketType")

    for _mkt_idx, _mkt in enumerate(markets):
        _smt = (_mkt.get("sportsMarketType") or "").lower()
        _q = (_mkt.get("question") or "").lower()

        # ─ MATCH WINNER (Full Match) ─
        if _smt == "moneyline":
            _oc = _jp(_mkt.get("outcomes", "[]"))
            _pr = _jp(_mkt.get("outcomePrices", "[]"))
            _tk = _jp(_mkt.get("clobTokenIds", "[]"))
            _prices = []
            for p in _pr:
                try: _prices.append(float(p))
                except: _prices.append(0.0)

            if len(_oc) == 2 and len(_prices) >= 2:
                home_win_name = str(_oc[0])
                home_win_price = _prices[0]
                home_win_tid = _tk[0] if _tk else ""
                away_win_name = str(_oc[1]) if len(_oc) > 1 else ""
                away_win_price = _prices[1] if len(_prices) > 1 else 0
                away_win_tid = _tk[1] if len(_tk) > 1 else ""
                logger.info(f"  ├─✅ MONEYLINE: {home_win_name}@{home_win_price} vs {away_win_name}@{away_win_price}")

        # ─ 1ST SET WINNER ─
        elif _smt in ("tennis_first_set_winner", "first_set_winner"):
            _oc = _jp(_mkt.get("outcomes", "[]"))
            _pr = _jp(_mkt.get("outcomePrices", "[]"))
            _tk = _jp(_mkt.get("clobTokenIds", "[]"))
            _prices = []
            ...
            if len(_oc) == 2 and len(_prices) >= 2 and not parsed.get("set1"):
                parsed["set1"] = {
                    "home": _prices[0], "home_tid": _tk[0] if _tk else "",
                    "home_name": str(_oc[0]),
                    "away": _prices[1] if len(_prices) > 1 else 0,
                    "away_tid": _tk[1] if len(_tk) > 1 else "",
                    "away_name": str(_oc[1]) if len(_oc) > 1 else "",
                }
                logger.info(f"  ├─✅ 1ST SET: {_oc[0]}@{_prices[0]} vs {_oc[1]}@{_prices[1]}")

    _preloop_ml_found = bool(home_win_price > 0 and home_win_tid)
    logger.info(f"  └─ PRE-LOOP: ML_FOUND={_preloop_ml_found}, Set1={bool(parsed.get('set1'))}")

    # ════════════════════════════════════════════════════════════════
    # MAIN LOOP: Parse spreads, totals, BTTS, etc.
    # (Skips ML/Set1 if already found via sportsMarketType)
    # ════════════════════════════════════════════════════════════════
    ...
    for mkt_idx, mkt in enumerate(markets):
        ...
        # ⭐ SKIP if already found via sportsMarketType
        if _preloop_ml_found and ("moneyline" in q or "match result" in q or "winner" in q):
            if not any(k in q for k in ["1st set", "2nd set", "first set"]):
                logger.info(f"  SKIP Mkt[{mkt_idx}]: Already have ML from pre-loop")
                continue

        if parsed.get("set1") and ("1st set" in q or "first set" in q):
            logger.info(f"  SKIP Mkt[{mkt_idx}]: Already have Set1 from pre-loop")
            continue

        # ─ Will patterns (football) ─
        ...
        # ─ Moneyline/Match Result patterns ─
            ...
            if len(prices) >= 2 and any(_price_valid(p) for p in prices[:2]):
                ...
                if existing_sp is None:
                    sp = {
                        ...
                        "home": fav_price, "home_tid": fav_tid, "home_name": fav_name,
                        "away": dog_price, "away_tid": dog_tid, "away_name": dog_name,
                        "home_line": f"-{line_num}", "away_line": f"+{line_num}",
                        "line": f"-{line_num}",
                    }
        ...
        # ─ Fallback: 2-outcome market ─
        elif not _preloop_ml_found and not home_win_price and not home_win_tid and len(outcomes) == 2 and \
            ...
            if _price_valid(prices[0] if prices else 0):
                ...
                if is_title_match:
                    ...
                    logger.info(f"  ML from API: outcomes={[str(o) for o in outcomes]} "
                                f"home='{home_win_name}'@{home_win_price} away='{away_win_name}'@{away_win_price}")
    ...
    if home_win_price > 0 or away_win_price > 0:
        ...
        # Safety: Clean up draw leakage
        if home_win_name and home_win_name.lower().strip() in ("draw", "tie"):
            logger.warning(f"  SAFETY: Draw leaked into HOME slot — moving to draw")
            if not draw_price:
        ...
        if away_win_name and away_win_name.lower().strip() in ("draw", "tie"):
            logger.warning(f"  SAFETY: Draw leaked into AWAY slot — moving to draw")
            if not draw_price:
    ...
    # Fallback: Use Mkt[0] if still no ML
    if not parsed["ml"] and markets:
        mkt0 = markets[0]
        ...
        if len(oc0) == 2 and oc_names0 != ["yes", "no"] and \
           not any(n in ("draw", "tie") for n in oc_names0) and \
           len(prices0) >= 2 and (_price_valid(prices0[0]) or _price_valid(prices0[1])):
            h_name = str(oc0[0])
            ...
            logger.info(f"  LAST RESORT ML from Mkt[0]: '{h_name}'@{prices0[0]} vs '{a_name}'@{prices0[1]}")

    parsed["volume"] = total_vol

    # ════════════════════════════════════════════════════════════════
    # REORDER: Ensure home slot = title's home team
    # ════════════════════════════════════════════════════════════════
    ...
    logger.info(f"  RESULT: ML={'\u2713' if parsed['ml'] else '\u2717'} Set1={'\u2713' if parsed.get('set1') else '\u2717'}")
    return parsed
