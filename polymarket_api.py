            ...
            if r.status_code == 200:
                ...
                if isinstance(data, list):
                    ...
                    if data and data[0].get("markets"):
                        m0 = data[0]["markets"][0]
                        logger.info(f"  Sample market: q={m0.get('question','')[:60]} outcomes={m0.get('outcomes','')[:60]} prices={m0.get('outcomePrices','')[:60]}")
                    return data
...
def format_match(g: dict, currency: str = "USD") -> str:
    ...
    if g.get("time"):
        try:
            ...
            parts.append(f"📅 {dt.strftime('%a %d %b %Y')}")
            parts.append(f"⏰ {dt.strftime('%H:%M')} UTC")
        except: pass
    if g.get("live"):
