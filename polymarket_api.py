...
def parse_event(event: dict) -> dict:
    ...
    # ══════════════════════════════════════════════════════════════
    # PRE-LOOP: Use sportsMarketType field to identify ML definitively
    # Polymarket labels every market: "moneyline", "tennis_first_set_winner",
    # "tennis_match_totals", "tennis_first_set_totals", "tennis_set_totals", etc.
    # This is the ONLY reliable way to identify the correct market.
    # ══════════════════════════════════════════════════════════════
    for _mkt in markets:
        ...
        if _smt == "moneyline":
            ...
            if len(_oc) == 2 and len(_prices) >= 2:
                ...
                logger.info(f"  ML via sportsMarketType: {home_win_name}@{home_win_price} vs {away_win_name}@{away_win_price}")
        ...
        # Set 1 Winner
            ...
            if len(_oc) == 2 and len(_prices) >= 2 and not parsed.get("set1"):
                ...
                logger.info(f"  Set1 via sportsMarketType: {_oc[0]}@{_prices[0]} vs {_oc[1] if len(_oc)>1 else '?'}@{_prices[1] if len(_prices)>1 else 0}")
    
    _preloop_ml_found = bool(home_win_price > 0 and home_win_tid)
    if _preloop_ml_found:
        logger.info(f"  ML LOCKED via sportsMarketType: {home_win_name}@{home_win_price} tid={home_win_tid[:20]}...")
