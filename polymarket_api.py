...
async def fetch_positions(wallet_address: str) -> list:
    """
    Fetch user's current positions from Polymarket Data API.
    This is the source of truth — on-chain data that survives any restart.
    Returns list of positions with size, avgPrice, currentValue, pnl, title, etc.
    """
    url = "https://data-api.polymarket.com/positions"
    params = {
        "user": wallet_address,
        "sizeThreshold": "0.01",  # Include even tiny positions
        "limit": "100",
        "sortBy": "CURRENT",
        "sortDirection": "DESC",
    }
    async with httpx.AsyncClient(timeout=15) as client:
        try:
            ...
            logger.info(f"fetch_positions({wallet_address[:10]}...): {r.status_code}")
            if r.status_code == 200:
                data = r.json()
...
def _is_expired(event: dict) -> bool:
    ...
    # Use game_start_time first (most accurate for sports), then startTime, then endDate
    t = event.get("game_start_time") or event.get("startTime") or event.get("endDate", "")
    if not t:
        return False
    try:
        ...
        # If event started more than 4 hours ago, consider it expired
        # (most sports games finish within 3-4 hours)
        if (now - event_time).total_seconds() > 4 * 3600:
            return True
    except:
        pass
    
    # Also check if all markets have settled prices (0.00 or 1.00)
    # This catches games that ended but haven't been marked closed yet
    # Use wider threshold (0.02-0.98) because near-settled markets may
    # show prices like 0.97/0.03 after a game ends
    markets = event.get("markets", [])
    if markets:
        all_settled = True
        for m in markets:
            # Check closed/resolved flags directly
            if str(m.get("closed", "")).lower() == "true" or str(m.get("resolved", "")).lower() == "true":
                continue  # This market is done
            ...
            if not all_settled:
                break
        if all_settled and len(markets) > 0:
            return True
...
async def fetch_token_price(token_id: str, side: str = "BUY") -> float:
    """Fetch current best price for a token from CLOB."""
    async with httpx.AsyncClient(timeout=10) as client:
        try:
            r = await client.get(
            ...
            logger.error(f"fetch_token_price: {e}")
    return 0.0
