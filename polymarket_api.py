...
def parse_event(event: dict) -> dict:
    ...
    parsed = {
        ...
        "set1": None,  # Set 1 Winner for tennis
    ...
    if sport != "soccer" and markets:
        # Find the ML market: the one whose question matches the event title
        # Sub-markets have different questions like "Set 1 Winner", "Set Handicap", "O/U"
        _sub_keywords = ["set 1", "set 2", "set 3", "set winner", "set handicap",
                         "1st set", "2nd set", "3rd set", "game handicap",
                         "total games", "total sets", "tiebreak",
                         "o/u", "over", "under", "total",
                         "handicap:", "spread", "half", "quarter",
                         "prop", "mvp", "champion", "futures"]
        title_lower = title.lower().replace(" - more markets", "").strip()
        
        for _mi, _mkt in enumerate(markets):
            _q = (_mkt.get("question") or "").strip()
            _q_lower = _q.lower().replace(" - more markets", "").strip()
            ...
            _oc_names = [str(o).lower().strip() for o in _oc]
            
            # Skip Yes/No markets, O/U, sub-markets
            if _oc_names == ["yes", "no"]:
                continue
            if any(k in _q_lower for k in _sub_keywords):
                # But capture Set 1 Winner for tennis as a separate market
                if ("set 1 winner" in _q_lower or "set winner" in _q_lower) and                    len(_oc) == 2 and _oc_names != ["yes", "no"] and                    len(_prices) >= 2 and _price_valid(_prices[0]):
                    parsed["set1"] = {
                    ...
                    logger.info(f"  Set 1 Winner captured: {_oc[0]}@{_prices[0]} vs {_oc[1]}@{_prices[1] if len(_prices)>1 else 0}")
                else:
                    logger.info(f"  Mkt[{_mi}] SKIP sub-market: {_q[:50]}")
                continue
            if len(_oc) != 2 or len(_prices) < 2:
                continue
            if not (_price_valid(_prices[0]) or _price_valid(_prices[1])):
                continue
            
            # This is a 2-outcome player-name market that isn't a sub-market = ML
            ...
            logger.info(f"  ML from Mkt[{_mi}] ({sport}): {home_win_name}@{home_win_price} vs {away_win_name}@{away_win_price} | q={_q[:50]}")
            break
    
    # Flag if pre-loop already found ML
    _preloop_ml_found = bool(home_win_price > 0 and home_win_tid)
    if _preloop_ml_found:
        logger.info(f"  PRE-LOOP ML LOCKED: {home_win_name}@{home_win_price} tid={home_win_tid[:20]}...")
    
    for mkt_idx, mkt in enumerate(markets):
        q = (mkt.get("question") or mkt.get("groupItemTitle") or "").lower()
        # DUMP: Log every market's key fields to find the differentiator
        _q_raw = mkt.get("question", "")
        _git = mkt.get("groupItemTitle", "")
        _slug = mkt.get("slug", "")
        _smt = mkt.get("sportsMarketType", "")
        if mkt_idx < 6:  # Only log first 6 markets
            logger.info(f"    Mkt[{mkt_idx}]: q={_q_raw[:60]} | git={_git[:40]} | slug={_slug[:40]} | smt={_smt}")
