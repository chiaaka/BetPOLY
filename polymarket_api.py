...
def parse_event(event: dict) -> dict:
    ...
    for mkt_idx, mkt in enumerate(markets):
        ...
        # PROFESSIONAL FIX: Only accept as moneyline if:
        #   a) It's the first market (Mkt[0]) — Polymarket always puts ML first, OR
        #   b) The question closely matches the event title (contains both team names)
        # This prevents "Set 1 Winner", "Set Handicap" etc from being picked as ML
        # ==============================================
        elif not home_win_price and len(outcomes) == 2 and \
             outcome_names != ["yes", "no"]:
            if _price_valid(prices[0] if prices else 0):
                # Check: is this Mkt[0] or does the question match the event title?
                q_lower = q.lower()
                title_lower = title.lower()
                
                is_first_market = (mkt_idx == 0)
                title_words_in_q = False
                if home and away:
                    # Check if both team/player names appear in the question
                    h_parts = home.lower().split()
                    a_parts = away.lower().split()
                    h_match = any(w in q_lower for w in h_parts if len(w) > 2)
                    a_match = any(w in q_lower for w in a_parts if len(w) > 2)
                    title_words_in_q = h_match and a_match
                
                # Also check: does the question contain sub-market keywords?
                is_sub_market = any(k in q_lower for k in [
                    "set 1", "set 2", "set 3", "set winner", "set handicap",
                    "1st set", "2nd set", "3rd set", "game handicap",
                    "total games", "total sets", "tiebreak",
                    "half", "quarter", "corners", "cards",
                    "handicap:", "spread:", "prop", "mvp",
                    "o/u", "over", "under", "total",
                    "champion", "futures", "season", "award",
                ])
                
                if is_sub_market:
                    logger.info(f"  SKIP sub-market in fallback: '{q[:60]}'")
                elif is_first_market or title_words_in_q:
                    ...
                    logger.info(f"  SKIP non-ML 2-outcome: '{q[:60]}' (not Mkt[0], title mismatch)")
