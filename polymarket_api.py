...
def _price_valid(p: float) -> bool:
    """Check if price is tradeable (not settled at 0 or 1)."""
    return 0.02 <= p <= 0.98
    ...
    for mkt in markets:
        ...
        if "win" in q and len(outcomes) == 2 and outcome_names == ["yes", "no"]:
            ...
            if _price_valid(yes_price):
                # Determine if this is home or away by checking team name in question
                is_home = False
                is_away = False
                if home:
                    # Check if any significant word from home team is in question
                    home_words = [w for w in home.lower().split() if len(w) > 3]
                    if any(w in q for w in home_words):
                        is_home = True
                if away and not is_home:
                    away_words = [w for w in away.lower().split() if len(w) > 3]
                    if any(w in q for w in away_words):
                        is_away = True
