...
def format_match(g: dict, currency: str = "USD") -> str:
    ...
    if g.get("time"):
        try:
            ...
            lines.append(f"⏰ {dt.strftime('%a %d %b, %H:%M')} UTC")
        except: pass
    ...
    if g["ml"]:
        ml = g["ml"]
        h = fmt_odds(ml["home"])
        d = fmt_odds(ml["draw"]) if ml["draw"] > 0 else ""
        a = fmt_odds(ml["away"])
    ...
    if g["totals"]:
        main = g["totals"][0]
        for t in g["totals"]:
            if str(t["line"]) in ["2.5", "3.5"]: main = t; break
    ...
    if g.get("volume"):
        try: lines.append(f"💰 Vol: {fmt_currency(float(g['volume']), currency)}")
        except: pass
