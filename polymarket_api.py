...
def parse_event(event: dict) -> dict:
    ...
    for mkt_idx, mkt in enumerate(markets):
        ...
        # ==============================================
             ...
             not any(k in q for k in ["prop", "mvp", "champion", "futures", "season", "award",
                                       "o/u", "over", "under", "total",
                                       "set 1", "set 2", "set 3", "set winner", "set handicap",
                                       "1st set", "2nd set", "3rd set",
                                       "game handicap", "total games", "total sets",
                                       "tiebreak", "half", "first", "second", "quarter",
                                       "handicap:", "spread:", "corners", "cards"]):
    ...
    if not parsed["ml"]:
        for mkt in markets:
            ...
            # Skip O/U, props, futures, set markets, handicaps
            if any(k in q for k in ["o/u", "over", "under", "total", "prop", "mvp",
                                      "champion", "futures", "season", "award",
                                      "set 1", "set 2", "set 3", "set winner",
                                      "set handicap", "1st set", "2nd set", "3rd set",
                                      "game handicap", "total games", "total sets",
                                      "tiebreak", "aces", "double fault",
                                      "half", "first", "second", "quarter",
                                      "halftime", "corners", "cards",
                                      "handicap:", "spread:"]):
                logger.debug(f"  SKIP sub-market: '{q[:60]}'")
                continue
            
            # Extra guard: skip any market with "set" followed by a number (tennis sub-markets)
            if re.search(r'\bset\s*\d', q) or re.search(r'\bset\s+(winner|handicap|score)', q):
                logger.info(f"  SKIP tennis set market: '{q[:60]}'")
                continue
            
            # Any 2-outcome market with valid prices and non-Yes/No outcomes = moneyline
            # USE API DATA DIRECTLY — outcomes[0] ↔ tokens[0] ↔ prices[0]
            # SAFETY: Skip if any outcome is "draw" or "tie"
            # CRITICAL: Only set ML if not already set — first valid market wins (Mkt[0] is always moneyline)
            if len(outcomes) == 2 and outcome_names != ["yes", "no"]:
                if any(n in ("draw", "tie") for n in outcome_names):
                    continue
                if len(prices) >= 2 and (_price_valid(prices[0]) or _price_valid(prices[1])):
                    # Only set if ML not already found
                    if parsed.get("ml") and parsed["ml"].get("home") and parsed["ml"].get("home") > 0:
                        logger.debug(f"  ML already set, skipping: '{q[:60]}'")
                        continue
                    h_name = str(outcomes[0])
                    h_price = prices[0]
                    h_tid = tokens[0] if tokens else ""
                    a_name = str(outcomes[1]) if len(outcomes) > 1 else ""
                    a_price = prices[1] if len(prices) > 1 else 0
                    a_tid = tokens[1] if len(tokens) > 1 else ""
                    ...
                    logger.info(f"  FALLBACK ML (API direct): '{h_name}'@{h_price} vs '{a_name}'@{a_price}")
                    break
