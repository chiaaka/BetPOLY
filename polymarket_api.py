...
def format_match(g: dict, currency: str = "USD") -> str:
    ...
    logger.info(f"  format_match: sport={sport} ml_label={labels['ml']} total_label={labels['total']} totals={len(g.get('totals',[]))}")
...
async def fetch_buy_prices_for_game(g: dict) -> dict:
    """Fetch actual BUY prices from CLOB for all tokens in a game.
    Returns dict of {token_id: buy_price}. Updates game dict in-place
    and adds spread warnings."""
    import httpx
    
    # Collect all token IDs from the game
    tokens = {}  # token_id -> (section, key) for updating
    
    ml = g.get("ml")
    if ml:
        if ml.get("home_tid"): tokens[ml["home_tid"]] = ("ml", "home")
        if ml.get("away_tid"): tokens[ml["away_tid"]] = ("ml", "away")
        if ml.get("draw_tid"): tokens[ml["draw_tid"]] = ("ml", "draw")
    
    for i, t in enumerate(g.get("totals", [])):
        if t.get("over_tid"): tokens[t["over_tid"]] = ("totals", i, "over")
        if t.get("under_tid"): tokens[t["under_tid"]] = ("totals", i, "under")
    
    btts = g.get("btts")
    if btts:
        if btts.get("yes_tid"): tokens[btts["yes_tid"]] = ("btts", "yes")
        if btts.get("no_tid"): tokens[btts["no_tid"]] = ("btts", "no")
    
    for i, s in enumerate(g.get("spreads", [])):
        if s.get("home_tid"): tokens[s["home_tid"]] = ("spreads", i, "home")
        if s.get("away_tid"): tokens[s["away_tid"]] = ("spreads", i, "away")
    ...
    async with httpx.AsyncClient(timeout=8) as client:
        for tid, path in tokens.items():
            try:
                # Fetch BUY price (what user actually pays)
                r = await client.get(
                ...
                if r.status_code == 200:
                    data = r.json()
                    buy_price = float(data.get("price", 0)) if isinstance(data, dict) else float(data or 0)
                    if buy_price > 0:
                        buy_prices[tid] = buy_price
                        
                        # Get the mid-price for spread calculation
                        mid_price = _get_mid_from_game(g, path)
                        if mid_price > 0 and buy_price > 0:
                            spread = abs(buy_price - mid_price) / mid_price
                            if spread > 0.05:  # >5% spread
                                wide_spreads.append(tid)
                        
                        # Update game dict with buy price
                        _set_price_in_game(g, path, buy_price)
            except Exception as e:
                logger.debug(f"Could not fetch buy price for {tid[:16]}...: {e}")
    
    # Add spread warning flag to game
    if wide_spreads:
        g["_wide_spread"] = True
        g["_wide_spread_count"] = len(wide_spreads)
    else:
        g["_wide_spread"] = False
    
    return buy_prices


def _get_mid_from_game(g: dict, path: tuple) -> float:
    """Get the current mid-price from game dict for a given path."""
    try:
        if path[0] == "ml":
            return g["ml"].get(path[1], 0)
        elif path[0] == "totals":
            return g["totals"][path[1]].get(path[2], 0)
        elif path[0] == "btts":
            return g["btts"].get(path[1], 0)
        elif path[0] == "spreads":
            return g["spreads"][path[1]].get(path[2], 0)
    except (IndexError, KeyError):
        pass
    return 0


def _set_price_in_game(g: dict, path: tuple, new_price: float):
    """Update price in game dict for a given path."""
    try:
        if path[0] == "ml":
            g["ml"][path[1]] = new_price
        elif path[0] == "totals":
            g["totals"][path[1]][path[2]] = new_price
        elif path[0] == "btts":
            g["btts"][path[1]] = new_price
        elif path[0] == "spreads":
            g["spreads"][path[1]][path[2]] = new_price
    except (IndexError, KeyError):
        pass
