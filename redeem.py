...
def _encode_ctf_redeem(condition_id: str) -> str:
    ...
    w3 = Web3()
    contract = w3.eth.contract(abi=ctf_redeem_abi)
    return contract.encode_abi("redeemPositions", args=[USDC_E, parent, cond_bytes, [1, 2]])


def _encode_neg_risk_redeem(condition_id: str, size: float) -> str:
    """
    Encode NegRiskAdapter.redeemPositions(conditionId, amounts)
    
    For neg-risk markets:
    - conditionId = the market's condition ID
    - amounts = [token_amount] — actual amount to redeem (NOT empty)
    """
    from web3 import Web3
    ...
    # Convert share size to raw token amount (USDC.e has 6 decimals)
    raw_amount = int(size * 1_000_000)
    amounts = [raw_amount]
    
    w3 = Web3()
    contract = w3.eth.contract(abi=neg_risk_abi)
    return contract.encode_abi("redeemPositions", args=[cond_bytes, amounts])
...
async def redeem_position(
    ...
    try:
        ...
        from py_builder_relayer_client import RelayerTxType
        ...
        relay_client = RelayClient(
            ...
            private_key,
            builder_config,
            tx_type=RelayerTxType.PROXY,  # Proxy wallet, NOT Safe
        )
        ...
        if neg_risk:
            ...
            call_data = _encode_neg_risk_redeem(condition_id, size)
            logger.info(f"Redeem: NegRisk market, condition={condition_id[:16]}..., size={size}")
        else:
        ...
        # Web3's encode_abi returns 0x-prefixed string already
        call_data_hex = call_data if call_data.startswith("0x") else "0x" + call_data
        ...
        response = relay_client.execute([tx], metadata="BetPoly: Redeem winning position")
