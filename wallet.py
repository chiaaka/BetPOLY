...
def _build_approval_txs():
    """
    Build 7 approval transactions matching polymarket.com docs.
    
    Uses the Transaction dict format from the official relayer docs:
      { to: string, data: string, value: string }
    
    The RelayClient.execute() internally converts these based on wallet type.
    """
    txs = []
...
def set_approvals(private_key: str) -> dict:
    ...
    try:
        ...
        # Try passing as list of dicts first (matches TS Transaction interface)
        try:
            response = client.execute(approval_txs, "Set all token approvals")
        except (TypeError, AttributeError) as te:
            # If plain dicts don't work, try SafeTransaction model
            logger.info("Plain dicts failed (%s), trying SafeTransaction model...", te)
            from py_builder_relayer_client.models import SafeTransaction
            safe_txs = []
            for tx in approval_txs:
                try:
                    safe_txs.append(SafeTransaction(
                        to=tx["to"], data=tx["data"], 
                        value=tx["value"], operation=0
                    ))
                except TypeError:
            ...
            response = client.execute(safe_txs, "Set all token approvals")
