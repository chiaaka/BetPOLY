...
3-signature flow matching polymarket.com:
  1. Deploy Safe (gasless)
  2. Set token approvals (gasless)
  3. Create CLOB API credentials (off-chain)
...
# =============================================================================
# Inspect + discover BuilderConfig and SafeTransaction on first import
# =============================================================================

try:
    from py_builder_signing_sdk.config import BuilderConfig
    _bc_src = inspect.getsource(BuilderConfig.__init__)
    logger.info("BuilderConfig.__init__ source:\n%s", _bc_src[:1500])
except Exception as _e:
    logger.error("Could not inspect BuilderConfig: %s", _e)

try:
    from py_builder_relayer_client.models import SafeTransaction
    _st_src = inspect.getsource(SafeTransaction)
    logger.info("SafeTransaction source:\n%s", _st_src[:1500])
except Exception as _e:
    logger.error("Could not inspect SafeTransaction: %s", _e)

try:
    from py_builder_relayer_client.client import RelayClient
    _rc_deploy = inspect.getsource(RelayClient.deploy)
    logger.info("RelayClient.deploy source:\n%s", _rc_deploy[:2000])
    _rc_exec = inspect.getsource(RelayClient.execute)
    logger.info("RelayClient.execute source:\n%s", _rc_exec[:2000])
except Exception as _e:
    logger.error("Could not inspect RelayClient: %s", _e)

try:
    from py_builder_relayer_client.builder.derive import derive as _derive_fn
    logger.info("derive source:\n%s", inspect.getsource(_derive_fn)[:1500])
except Exception as _e:
    logger.error("Could not inspect derive: %s", _e)
...
# =============================================================================
# Builder config — try multiple patterns until one works
# =============================================================================

def _get_builder_config():
    """Create BuilderConfig — tries multiple constructor patterns."""
    from py_builder_signing_sdk.config import BuilderConfig

    # Pattern 1: TypeScript-style local_builder_creds dict
    try:
    ...
    # Pattern 2: localBuilderCreds (camelCase)
    try:
        return BuilderConfig(
            localBuilderCreds={
                "key": BUILDER_API_KEY,
    ...
    # Pattern 3: key/secret/passphrase directly
    try:
        return BuilderConfig(
            key=BUILDER_API_KEY,
            secret=BUILDER_SECRET,
    ...
    # Pattern 4: api_key/api_secret/api_passphrase
    try:
        return BuilderConfig(
    ...
    # Pattern 5: positional args
    try:
        return BuilderConfig(BUILDER_API_KEY, BUILDER_SECRET, BUILDER_PASSPHRASE)
    except TypeError:
        pass

    # Pattern 6: no args, set after
    try:
        cfg = BuilderConfig()
        cfg.api_key = BUILDER_API_KEY
        cfg.secret = BUILDER_SECRET
        cfg.passphrase = BUILDER_PASSPHRASE
        return cfg
    except Exception:
        pass

    raise RuntimeError("Cannot create BuilderConfig — check logs for __init__ source")
...
def derive_safe_address(private_key: str) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.error("ClobClient fallback: %s", e2)
...
def _build_approval_txs():
    """Build 7 approval transactions using actual SafeTransaction model."""
    from py_builder_relayer_client.models import SafeTransaction
    txs = []
    for spender in [CTF, CTF_EXCHANGE, NEG_RISK_EXCHANGE, NEG_RISK_ADAPTER]:
        txs.append(SafeTransaction(to=USDC_E, data=_encode_approve(spender), value="0"))
    for op in [CTF_EXCHANGE, NEG_RISK_EXCHANGE, NEG_RISK_ADAPTER]:
        txs.append(SafeTransaction(to=CTF, data=_encode_set_approval_for_all(op), value="0"))
    return txs
...
def deploy_safe(private_key: str) -> dict:
    ...
    try:
        ...
        if hasattr(response, 'wait'):
            ...
            safe_addr = getattr(result, 'proxy_address', None) or getattr(result, 'proxyAddress', None)
...
def onboard_wallet(private_key: str) -> dict:
    """
    Complete onboarding — 3 gasless signatures matching polymarket.com.
    Called during /start signup.
    """
    result = {
