    ...
    try:
        if token_id:
            ...
            if verification:
                ...
                # Check if this looks like a player prop when user expects moneyline
                ...
                is_prop = any(k in market_name.lower() for k in prop_keywords)
                ...
                if is_prop and is_ml_bet:
                    # DANGER: User thinks they're betting moneyline but token is a prop
                    logger.error(f"  ❌ TOKEN MISMATCH: label='{bet.get('label')}' but token market='{market_name}'")
                    await q.edit_message_text(
                        "🚫 <b>BET BLOCKED — Safety Check Failed</b>\n\n"
                        f"You selected: <b>{esc(bet.get('label', ''))}</b>\n"
                        f"But the market is actually:\n"
                        f"<b>{esc(market_name)}</b>\n\n"
                        "This is a data mismatch. The bet was NOT placed\n"
                        "to protect your funds.\n\n"
                        "Please go back and try again — the odds\n"
                        "may have refreshed correctly now.",
                        parse_mode=ParseMode.HTML,
                        reply_markup=Markup([
        ...
        logger.debug(f"Token verification failed (non-fatal): {e}")
