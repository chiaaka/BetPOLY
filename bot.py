...
async def pin_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Deploy Polymarket Safe wallet (gasless via relayer)
    wallet_address = w["address"]  # fallback to EOA
    try:
        setup_msg = await update.effective_chat.send_message(
            "⏳ Setting up your smart wallet...", parse_mode=ParseMode.HTML
        )
        onboard = wallet.onboard_wallet(w["private_key"])
        if onboard.get("safe_address"):
            wallet_address = onboard["safe_address"]
            logger.info(f"Safe wallet for {user.id}: {wallet_address}")
        else:
            logger.warning(f"Safe deploy failed for {user.id}: {onboard.get('error')}")
        try:
            await setup_msg.delete()
        except:
            pass
    except Exception as e:
        logger.warning(f"Safe onboard error for {user.id}: {e}")
    
    await db.create_user(user.id, user.username or "", wallet_address, db.hash_pin(pin, user.id),
    ...
    key_msg = await update.effective_chat.send_message(
        ...
        f"Send USDC.e (Polygon) to the deposit address.\n\n"
...
async def onboard_currency(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("obc_"):
        ...
        addr = u["wallet_address"] if u else ""
...
# ========================================
# Position Monitor — Background Alerts
# ========================================

async def _monitor_positions(ctx: ContextTypes.DEFAULT_TYPE):
    """
    Background job that runs every 2 minutes.
    Checks all users' positions for changes and sends alerts:
    - 🏆 Won + Redeemable
    - ✅ Won (pending resolution)
    - ❌ Lost
    """
    from polymarket_api import fetch_positions
    
    try:
        users = await db.get_all_users_with_wallets()
    except Exception as e:
        logger.error(f"Monitor: failed to get users: {e}")
        return
    ...
    for u in users:
        ...
        try:
            positions = await fetch_positions(addr)
        except Exception as e:
            logger.warning(f"Monitor: fetch failed for {tid}: {e}")
            continue
        ...
        for p in positions:
            ...
            initial_val = p.get("initialValue", 0) or 0
            cash_pnl = p.get("cashPnl", 0) or 0
            avg_price = p.get("avgPrice", 0) or 0
            bought_odds = round(1 / avg_price, 2) if avg_price > 0 else 0
            
            alert_type = None
            alert_text = None
            
            # Check redeemable first (highest priority)
            if redeemable:
                alert_type = "redeemable"
                payout = round(size * 1.0, 2)
                profit = round(payout - initial_val, 2)
                alert_text = (
                    f"🏆 <b>YOU WON! Payout Ready!</b>\n\n"
                    f"<b>{outcome}</b> — {title}\n\n"
                    f"📊 {size:.1f} shares @ {bought_odds:.2f}\n"
                    f"💰 Invested: ${initial_val:.2f}\n"
                    f"🎉 Payout: ${payout:.2f}\n"
                    f"📈 Profit: ${profit:+.2f}\n\n"
                    f"Your winnings are ready to redeem!\n"
                    f"Check <b>My Bets</b> to claim."
                )
            
            # Won but not yet redeemable
            elif cur_price >= 0.98:
                alert_type = "won"
                payout = round(size * 1.0, 2)
                profit = round(payout - initial_val, 2)
                alert_text = (
                    ...
                    f"💰 Invested: ${initial_val:.2f}\n"
                    f"📈 Profit: ${profit:+.2f}\n\n"
                    f"⏳ Awaiting market resolution.\n"
                    f"You'll be notified when payout is ready!"
                )
            ...
            # Lost
                ...
                alert_text = (
                    ...
                    f"<b>{outcome}</b> — {title}\n\n"
                    f"📊 {size:.1f} shares @ {bought_odds:.2f}\n"
                    f"💰 Lost: ${initial_val:.2f}\n\n"
                    f"Better luck next time! 🎯\n"
                    f"Check /today for new matches."
                )
            
            # Send alert if we haven't already
            if alert_type and alert_text:
                already_sent = await db.has_alert_been_sent(tid, asset, alert_type)
                if not already_sent:
                    try:
                        kb = Markup([[Btn("💰 My Bets", callback_data="my_bets")]])
                        await ctx.bot.send_message(
                            tid, alert_text,
                            parse_mode=ParseMode.HTML,
                            reply_markup=kb
                        )
                        await db.mark_alert_sent(tid, asset, alert_type)
                        logger.info(f"Monitor: sent {alert_type} alert to {tid} for {title[:30]}")
                    except Exception as e:
                        logger.warning(f"Monitor: failed to send alert to {tid}: {e}")
...
async def _build_my_bets_text(wallet_address: str, cur: str) -> tuple:
    """
    Build My Bets text from Polymarket Data API (source of truth).
    Returns (text, sellable_positions) where sellable_positions only includes
    positions that can actually be sold (active, with tradeable prices).
    """
    from polymarket_api import fetch_positions
    ...
    for p in positions:
        ...
        # Classify: settled if redeemable, price at 0/1, or market ended
        if redeemable or cur_price <= 0.02 or cur_price >= 0.98:
            settled_bets.append(p)
        else:
            active_bets.append(p)
    
    if not active_bets and not settled_bets:
        return "💰 <b>My Bets</b>\n\nNo positions found.\nPick a match to get started! 👇", []
    ...
    if active_bets:
        ...
        for p in active_bets:
            ...
            cur_price = p.get("curPrice", 0) or 0
            initial_val = p.get("initialValue", 0) or 0
            current_val = p.get("currentValue", 0) or 0
            cash_pnl = p.get("cashPnl", 0) or 0
            pnl_emoji = "📈" if cash_pnl >= 0 else "📉"
            
            # Convert prices to decimal odds (what users see when betting)
            bought_odds = round(1 / avg_price, 2) if avg_price > 0 else 0
            now_odds = round(1 / cur_price, 2) if cur_price > 0 else 0
            
            text += f"\n⏳ <b>{outcome}</b> — {title[:35]}\n"
            text += f"  {size:.1f} shares | Bought @ {bought_odds:.2f} (now {now_odds:.2f})\n"
            text += f"  💰 ${initial_val:.2f} → ${current_val:.2f}"
            text += f"  {pnl_emoji} {cash_pnl:+.2f}\n"
    ...
    if settled_bets:
        ...
        for p in settled_bets:
            ...
            text += f"\n{status}\n"
            text += f"  <b>{outcome}</b> — {title[:35]}\n"
            text += f"  {size:.1f} shares @ {bought_odds:.2f} | Cost: ${initial_val:.2f}\n"
            if cash_pnl != 0:
                pnl_emoji = "📈" if cash_pnl >= 0 else "📉"
                text += f"  {pnl_emoji} P&L: ${cash_pnl:+.2f}\n"
        text += "\n"
    
    if not active_bets:
        text += "\n<i>No active positions. Place a bet to get started!</i>\n"
    
    return text, sellable, settled_bets
...
async def cmd_bets(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("💰 <b>My Bets</b>\n\n⏳ Loading from Polymarket...", parse_mode=ParseMode.HTML)
    
    text, sellable, settled = await _build_my_bets_text(addr, cur)
    
    rows = []
    # Sell buttons for active positions
    for i, p in enumerate(sellable[:10]):
        label = p.get('title', p.get('outcome', '?'))[:25]
        rows.append([Btn(f"💸 Sell: {label}", callback_data=f"sell_pos_{i}")])
    
    # Redeem buttons for won/redeemable positions
    for i, p in enumerate(settled[:10]):
        if p.get("redeemable"):
            label = p.get('title', p.get('outcome', '?'))[:22]
            rows.append([Btn(f"🏆 Redeem: {label}", callback_data=f"redeem_pos_{i}")])
    
    # Store for callbacks
    ctx.user_data["api_positions"] = sellable
    ctx.user_data["api_settled"] = settled
    
    rows.append([Btn("🔄 Refresh", callback_data="my_bets")])
    rows.append([Btn("🔙 Menu", callback_data="main_menu")])
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for flag in ["awaiting_stake", "awaiting_bet_pin", "awaiting_export", "awaiting_sell_pin", "awaiting_redeem_pin"]:
    ...
    # --- Wallet ---
        ...
        if user:
            ...
            try:
                ...
                if matic_bal < 0.01 and total > 0:
                    bal_text += "\n\n⚠️ <i>Low POL for gas. You need ~0.01 POL.</i>"
        ...
        if user:
            await q.edit_message_text(
                ...
                f"• No gas fees needed (gasless trading!)\n"
                f"• Min bet: $1 USDC",
                parse_mode=ParseMode.HTML, reply_markup=kb_wallet()
            )
    ...
    # --- My Bets ---
        ...
        if user:
            addr = user.get("wallet_address", "")
            await q.edit_message_text("💰 <b>My Bets</b>\n\n⏳ Loading from Polymarket...", parse_mode=ParseMode.HTML)
    ...
    # --- Sell Bet (show confirmation) ---
    # --- Sell Position (from Data API, show confirmation) ---
    elif d.startswith("sell_pos_"):
        idx = int(d.split("_")[2])
        positions = ctx.user_data.get("api_positions", [])
        if idx >= len(positions):
            await q.answer("Position not found. Try refreshing.", show_alert=True)
            return
        ...
        text = f"💸 <b>Sell Position</b>\n\n"
        text += f"<b>{p.get('outcome', '?')}</b> — {p.get('title', '?')[:40]}\n\n"
        bought_odds = round(1 / avg_price, 2) if avg_price > 0 else 0
        now_odds = round(1 / cur_price, 2) if cur_price > 0 else 0
        text += f"Shares: {size:.2f} | Bought @ {bought_odds:.2f}\n"
        text += f"Cost: ${initial_val:.2f}\n\n"
        text += f"Current odds: {now_odds:.2f}\n"
        text += f"Sell value: ~${current_val:.2f}\n"
        text += f"{pnl_emoji} P&L: ${cash_pnl:+.2f}\n\n"
        text += "🔐 <b>Enter your 6-digit PIN to confirm sale:</b>"
        
        # Store sell details for PIN confirmation
        ctx.user_data["pending_sell"] = {
            "asset": p.get("asset", ""),
            "token_id": p.get("asset", ""),  # asset IS the token_id
            "size": size,
            "title": p.get("title", ""),
            "outcome": p.get("outcome", ""),
        ...
        kb = Markup([[Btn("❌ Cancel", callback_data="my_bets")]])
        await q.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb)
    
    # --- Redeem Position (from settled positions) ---
    elif d.startswith("redeem_pos_"):
        idx = int(d.split("_")[2])
        settled = ctx.user_data.get("api_settled", [])
        if idx >= len(settled):
            await q.answer("Position not found. Try refreshing.", show_alert=True)
            return
        ...
        if not p.get("redeemable"):
            await q.answer("This position is not yet redeemable.", show_alert=True)
            return
        ...
        initial_val = p.get("initialValue", 0) or 0
        cur_price = p.get("curPrice", 0) or 0
        payout = round(size * cur_price, 2)
        profit = round(payout - initial_val, 2)
        
        text = f"🏆 <b>Redeem Winnings</b>\n\n"
        text += f"<b>{outcome}</b> — {title[:40]}\n\n"
        text += f"Shares: {size:.2f}\n"
        text += f"Invested: ${initial_val:.2f}\n"
        text += f"Payout: ~${payout:.2f}\n"
        text += f"📈 Profit: ~${profit:+.2f}\n\n"
        text += "🔐 <b>Enter your 6-digit PIN to redeem:</b>"
        
        # Use sell-at-market approach (CLOB holds the tokens, not EOA)
        ctx.user_data["pending_redeem"] = {
            "asset": p.get("asset", ""),
...
async def _confirm_bet(q, data: str, cur: str, ctx):
    ...
    try:
        ...
        result = await trade.place_bet(
            ...
            selection=bet["label"],
            funder_address=user.get("wallet_address"),
        )
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower() in ("cancel", "stop", "back", "menu", "/cancel", "/menu", "/start"):
        for flag in ["awaiting_stake", "awaiting_bet_pin", "awaiting_export",
                     "awaiting_sell_pin", "awaiting_redeem_pin", "pending_confirm_stake", "pending_sell", "pending_redeem", "bet"]:
            ctx.user_data.pop(flag, None)
        await update.message.reply_text(
    ...
    if ctx.user_data.get("awaiting_sell_pin"):
        ...
        sell_info = ctx.user_data.pop("pending_sell", None)
        if not sell_info:
            await update.effective_chat.send_message("❌ No pending sale.", reply_markup=kb_main())
            return
        
        msg = await update.effective_chat.send_message("💸 <b>Selling position...</b>\n\n⏳ Please wait...", parse_mode=ParseMode.HTML)
        
        try:
            w = wallet.derive_wallet(update.effective_user.id, pin)
            from trade import sell_position
            result = await sell_position(
                private_key=w["private_key"],
                funder_address=user["wallet_address"],
                token_id=sell_info["token_id"],
                shares=sell_info["size"],
            )
            
            if result.get("success"):
                sold_amount = result.get("amount", 0)
                text = f"✅ <b>Position Sold!</b>\n\n"
                text += f"<b>{sell_info.get('outcome', '?')}</b> — {sell_info.get('title', '')[:30]}\n"
                text += f"Shares sold: {sell_info.get('size', 0):.2f}\n"
                text += f"Proceeds: ~${sold_amount:.2f}\n"
                text += f"\n💰 Funds returned to your wallet."
            else:
                text = f"❌ <b>Sell Failed</b>\n\n{result.get('error', 'Unknown error')}"
        except Exception as e:
            logger.error(f"Sell PIN flow error: {e}")
            text = f"❌ <b>Sell Error</b>\n\n{str(e)[:100]}"
        
        kb = Markup([[Btn("💰 My Bets", callback_data="my_bets")], [Btn("🔙 Menu", callback_data="main_menu")]])
        await msg.edit_text(text, parse_mode=ParseMode.HTML, reply_markup=kb)
        return
    
    # --- REDEEM PIN ---
    # Gasless redeem via Polymarket's official Builder Relayer
    ...
    if ctx.user_data.get("awaiting_redeem_pin"):
        ...
        if db.hash_pin(pin, update.effective_user.id) != user["pin_hash"]:
            await update.effective_chat.send_message("❌ Wrong PIN.", reply_markup=kb_main())
            ctx.user_data.pop("pending_redeem", None)
            return
        
        redeem_info = ctx.user_data.pop("pending_redeem", None)
        if not redeem_info:
            await update.effective_chat.send_message("❌ No pending redemption.", reply_markup=kb_main())
            return
        
        msg = await update.effective_chat.send_message(
            "🏆 <b>Redeeming winnings...</b>\n\n⏳ Submitting to Polymarket relayer (gasless)...", 
            parse_mode=ParseMode.HTML
        )
        
        try:
            w = wallet.derive_wallet(update.effective_user.id, pin)
            from redeem import redeem_position
            result = await redeem_position(
                private_key=w["private_key"],
                funder_address=user["wallet_address"],
                condition_id=redeem_info["condition_id"],
                token_id=redeem_info["token_id"],
                size=redeem_info["size"],
                neg_risk=redeem_info.get("neg_risk", False),
            )
            
            if result.get("success"):
                payout = redeem_info.get("payout", 0)
                profit = round(payout - (redeem_info.get("initial_val", 0) or 0), 2)
                text = f"🏆 <b>Winnings Redeemed!</b>\n\n"
                text += f"<b>{redeem_info.get('outcome', '?')}</b> — {redeem_info.get('title', '')[:30]}\n\n"
                text += f"💰 ${payout:.2f} USDC.e returned to your wallet!\n"
                text += f"📈 Profit: ${profit:+.2f}\n"
                if result.get("tx_hash"):
                    text += f"\n🔗 <a href='https://polygonscan.com/tx/{result['tx_hash']}'>View on PolygonScan</a>"
            else:
                text = f"❌ <b>Redeem Failed</b>\n\n{result.get('error', 'Unknown error')}"
        except Exception as e:
            logger.error(f"Redeem PIN flow error: {e}")
            text = f"❌ <b>Redeem Error</b>\n\n{str(e)[:100]}"
...
def main():
    ...
    async def post_init(application):
        ...
        # Start position monitor — checks every 2 minutes for won/lost/redeemable
        ...
        logger.info("🚀 BetPoly is running! (Position monitor active)")
    ...
    app.add_handler(CommandHandler("mybets", cmd_bets))
    app.add_handler(CommandHandler("mybet", cmd_bets))
    app.add_handler(CommandHandler("mybets", cmd_bets))
    app.add_handler(CommandHandler("mybet", cmd_bets))
