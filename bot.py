        ...
        if user:
            ...
            try:
                ...
                if pin:
                    ...
                    usdc_bal = trade.get_usdc_balance(w["private_key"])
                    matic_bal = trade.get_matic_balance(w["private_key"])
                    bal_text = (
                        f"💰 USDC: <b>${usdc_bal:.2f}</b> ({fmt_currency(usdc_bal, cur)})\n"
                        f"⛽ POL (gas): <b>{matic_bal:.4f}</b>"
                    )
                else:
                    bal_text = "💰 <i>Enter PIN to check live balance</i>"
            except:
                bal_text = "💰 <i>Balance check unavailable</i>"
