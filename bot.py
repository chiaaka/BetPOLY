...
def kb_game_odds(game: dict, idx: int, league: str):
    ...
    if game.get("totals"):
        from polymarket_api import _pick_main_total
        sport = game.get("sport", "soccer")
        main = _pick_main_total(game["totals"], sport)
        ...
        # Other lines
        for ti, t in enumerate(game["totals"]):
            if t == main:
                continue
            r = []
            if t["over"] > 0:
                r.append(Btn(f"Ov {t['line']} │ {fmt_odds(t['over'])}", callback_data=f"b_{league}_{idx}_t_{ti}_o"))
            if t["under"] > 0:
                r.append(Btn(f"Un {t['line']} │ {fmt_odds(t['under'])}", callback_data=f"b_{league}_{idx}_t_{ti}_u"))
            if r:
                rows.append(r)
    ...
    # Handicap / Spread — ALL lines with correct +/-
    if game.get("spreads"):
        for si, sp in enumerate(game["spreads"]):
            ...
            r = []
            if sp["home"] > 0:
                r.append(Btn(f"H1 ({h_line}) │ {fmt_odds(sp['home'])}", callback_data=f"b_{league}_{idx}_sp_{si}_h"))
            if sp["away"] > 0:
                r.append(Btn(f"H2 ({a_line}) │ {fmt_odds(sp['away'])}", callback_data=f"b_{league}_{idx}_sp_{si}_a"))
            if r:
                rows.append(r)
