...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Wallet ---
        ...
        if user:
            ...
            # Direct buy links — user completes purchase on provider's site
            # USDC arrives to their wallet on Polygon
            moonpay_url = (
            ...
            # Onramper aggregates 25+ providers — picks best for user's country
            # Works with Apple Pay, credit card, bank transfer, PayPal, Revolut
            ONRAMPER_KEY = os.environ.get("ONRAMPER_API_KEY", "")
            if ONRAMPER_KEY:
                onramper_url = (
                    f"https://buy.onramper.com/?"
                    f"apiKey={ONRAMPER_KEY}"
                    f"&defaultCrypto=USDC_POLYGON"
                    f"&onlyCryptos=USDC_POLYGON"
                    f"&onlyCryptoNetworks=POLYGON"
                    f"&networkWallets=POLYGON:{deposit_addr}"
                )
            else:
                onramper_url = None
            ...
            buttons = []
            if onramper_url:
                buttons.append([Btn("💳 Buy USDC →", url=onramper_url)])
            buttons.append([Btn("🌙 Buy on MoonPay →", url=moonpay_url)])
            buttons.append([Btn("💰 I already have crypto", callback_data="deposit_crypto")])
            buttons.append([Btn("🔙 Back", callback_data="w_deposit")])
            
            await q.edit_message_text(
                "💳 <b>Buy USDC</b>\n\n"
                "Buy USDC with your preferred payment method.\n"
                "It will be sent to your betting wallet on Polygon.\n\n"
                ...
                "<i>One-time verification required on first purchase.\n"
                "After that, future deposits take ~2 minutes.</i>",
                parse_mode=ParseMode.HTML,
