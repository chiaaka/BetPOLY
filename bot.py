            ...
            if book.get("bids"):
                ...
                if est["total_cost"] > 0:
                    ...
                    if est["avg_price"] > 0:
                        ...
                        # For sells: current_price is best bid, avg_price is what you actually get
                        # Lower avg_price = worse for seller
                        slippage_pct = 0
                        if current_price > 0 and est["avg_price"] > 0:
                            slippage_pct = ((current_price - est["avg_price"]) / current_price) * 100
                        
                        MAX_SELL_SLIPPAGE = 10
                        if slippage_pct > MAX_SELL_SLIPPAGE:
                            logger.warning(f"  🛑 SELL SLIPPAGE BLOCK: best_bid={current_price:.4f} avg_fill={est['avg_price']:.4f} slip={slippage_pct:.1f}%")
                            await q.edit_message_text(
                                "🛑 <b>SELL BLOCKED — Slippage Protection</b>\n\n"
                                f"📋 {bet_row.get('match_name', '?')}\n"
                                f"✅ {bet_row.get('selection', '?')}\n\n"
                                f"📊 Best bid price: <b>${current_price:.2f}</b>\n"
                                f"📊 Actual fill price: <b>${est['avg_price']:.2f}</b>\n"
                                f"📉 Slippage: <b>{slippage_pct:.1f}%</b> (max {MAX_SELL_SLIPPAGE}%)\n\n"
                                f"Selling {shares:.2f} shares would eat through\n"
                                f"too many orderbook levels.\n\n"
                                "💡 Wait for more buyers or try selling\n"
                                "fewer shares at a time.",
                                parse_mode=ParseMode.HTML,
                                reply_markup=Markup([
                                    [Btn("🔄 Refresh", callback_data=f"sell_{bet_id}"),
                                     Btn("❌ Cancel", callback_data="my_bets")]
                                ])
                            )
                            return
...
async def _show_confirm(q, stake: float, bet: dict, cur: str):
    ...
    fill_warning = ""
    MAX_SLIPPAGE_PCT = 10  # Block bet if fill is >10% worse than displayed odds
    try:
        ...
        if token_id:
            ...
            if book.get("asks"):
                ...
                if est["avg_price"] > 0 and est["filled"]:
                    ...
                    # Calculate slippage from displayed odds
                    ...
                    if displayed_odds > 0 and est_odds > 0:
                        # Lower odds = worse for buyer (paying more per share)
                        slippage_pct = ((displayed_odds - est_odds) / displayed_odds) * 100
                    
                    # SLIPPAGE PROTECTION: Block if fill is too far from displayed
                    if slippage_pct > MAX_SLIPPAGE_PCT:
                        logger.warning(f"  🛑 SLIPPAGE BLOCK: displayed={displayed_odds:.2f} est_fill={est_odds:.2f} slip={slippage_pct:.1f}%")
                        await q.edit_message_text(
                            "🛑 <b>BET BLOCKED — Slippage Protection</b>\n\n"
                            f"📋 {esc(bet['match'])}\n"
                            f"✅ {esc(bet['label'])}\n\n"
                            f"📊 Displayed odds: <b>{displayed_odds:.2f}</b>\n"
                            f"📊 Actual fill odds: <b>{est_odds:.2f}</b>\n"
                            f"📉 Slippage: <b>{slippage_pct:.1f}%</b> (max {MAX_SLIPPAGE_PCT}%)\n\n"
                            f"The orderbook is too thin for ${stake:.2f}.\n"
                            f"Available liquidity: ${est['depth_usdc']:.2f}\n\n"
                            "💡 Try a smaller amount or wait for\n"
                            "more liquidity.",
                            parse_mode=ParseMode.HTML,
                            reply_markup=Markup([
                                [Btn("💵 Try $1", callback_data="stake_1"),
                                 Btn("💵 Try $5", callback_data="stake_5")],
                                [Btn("🔄 Refresh", callback_data=bet.get("cb", "main_menu")),
                    ...
                    # Show mild warning if slippage is noticeable but within limits
                    if slippage_pct > 3:
                        fill_warning = f"⚠️ <i>Slippage: {slippage_pct:.1f}% (displayed {displayed_odds:.2f} → fill {est_odds:.2f})</i>\n\n"
                    
                elif not est["filled"]:
                    # Not enough liquidity to fill at all
                    await q.edit_message_text(
                        "🛑 <b>BET BLOCKED — Insufficient Liquidity</b>\n\n"
                        f"📋 {esc(bet['match'])}\n"
                        f"✅ {esc(bet['label'])}\n\n"
                        f"💰 You want: <b>${stake:.2f}</b>\n"
                        f"📊 Available: <b>${est['depth_usdc']:.2f}</b>\n\n"
                        "Not enough shares on the orderbook.\n"
