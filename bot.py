...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Wallet ---
        ...
        if user:
            ...
            # Guardarian — works in Nigeria, no registration needed
            guardarian_url = (
                f"https://guardarian.com/buy/usdc"
                f"?to_currency=USDC"
                f"&to_network=POLYGON"
                f"&to_address={deposit_addr}"
            )
            
            # Changelly — global, credit card + apple pay
            changelly_url = (
                f"https://changelly.com/buy/usdc"
                f"?currencyTo=usdc"
                f"&amountTo=10"
                f"&address={deposit_addr}"
            )
            
            buttons = [
                [Btn("💳 Buy on Guardarian →", url=guardarian_url)],
                [Btn("💳 Buy on Changelly →", url=changelly_url)],
                [Btn("💰 I already have crypto", callback_data="deposit_crypto")],
                [Btn("🔙 Back", callback_data="w_deposit")],
            ]
            
            await q.edit_message_text(
                "💳 <b>Buy USDC</b>\n\n"
                "Tap a button to buy USDC. It gets sent "
                "straight to your betting wallet.\n\n"
                "🟢 <b>Guardarian</b> — Works in Nigeria\n"
                "   Card, Apple Pay, bank transfer\n\n"
                "🔵 <b>Changelly</b> — Global\n"
                "   Card, Apple Pay, Google Pay\n\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━\n"
                "📬 Your wallet (Polygon):\n"
                f"<code>{deposit_addr}</code>\n\n"
                "<i>First purchase requires quick ID check.\n"
                "After that, ~2 min per deposit.</i>",
                parse_mode=ParseMode.HTML,
