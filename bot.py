...
from live_scores import fetch_soccer_scores, fetch_nba_scores, match_score_to_game, format_score_line
            ...
            try:
                ...
                if sport == "soccer":
                    ...
                    if not match:
                        ...
                        import asyncio
                        all_leagues = ["epl", "lal", "sea", "bun", "fl1", "ucl", "uel", "lig", "mls"]
        ...
        try:
            ...
            if book.get("bids"):
                ...
                if est["total_cost"] > 0:
                    ...
                    if not est["filled"]:
                        fill_warning = f"\n⚠️ <i>Not enough liquidity to sell all {shares:.2f} shares. Only {est['depth_usdc']:.2f} USDC available on buy side.</i>\n"
                    elif abs(est["avg_price"] - current_price) / max(current_price, 0.01) > 0.10:
                        fill_warning = f"\n⚠️ <i>Low liquidity — actual sell price may differ from displayed.</i>\n"
        except Exception as e:
...
async def _show_game(q, league: str, idx: int, cur: str, ctx):
    ...
    try:
        ...
        if sport == "soccer":
            ...
            if not match:
                import asyncio
                all_leagues = ["epl", "lal", "sea", "bun", "fl1", "ucl", "uel", "lig", "mls", "erd", "spl", "bel", "sup"]
                tasks = [fetch_soccer_scores(lg) for lg in all_leagues]
                results = await asyncio.gather(*tasks, return_exceptions=True)
            ...
            if book.get("asks"):
                ...
                if est["avg_price"] > 0 and est["filled"]:
                    ...
                    if displayed_odds > 0 and est_odds > 0:
                        ...
                        if diff_pct > 10:
                            fill_warning = f"⚠️ <i>Est. fill differs from displayed odds ({displayed_odds:.2f} → {est_odds:.2f}). Low liquidity.</i>\n\n"
                elif not est["filled"]:
