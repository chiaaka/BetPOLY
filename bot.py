...
async def _build_my_bets_text(wallet_address: str, cur: str) -> tuple:
    ...
    Returns (text, positions_list) where positions_list can be used for sell buttons.
    """
    ...
    for p in positions:
        ...
        text += f"  📊 {size:.1f} shares @ {avg_price:.3f}\n"
        text += f"  💰 Cost: ${initial_val:.2f} → Now: ${current_val:.2f}\n"
        text += f"  {pnl_emoji} P&L: ${cash_pnl:+.2f} ({pct_pnl:+.1f}%)\n"
        text += f"  {status}\n\n"
...
async def cmd_bets(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text, positions = await _build_my_bets_text(addr, cur)
    
    rows = []
    for i, p in enumerate(positions[:10]):
        if p.get("redeemable"):
            rows.append([Btn(f"🏆 Redeem: {p.get('outcome','?')[:20]}", callback_data=f"redeem_pos_{i}")])
        elif (p.get("curPrice", 0) or 0) > 0.02:
            rows.append([Btn(f"💸 Sell: {p.get('outcome','?')[:20]}", callback_data=f"sell_pos_{i}")])
    
    # Store positions for sell/redeem callbacks
    ctx.user_data["api_positions"] = positions
