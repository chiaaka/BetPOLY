...
from polymarket_api import (
    ...
    format_match, price_to_odds, fmt_odds, fmt_currency, refresh_currency_rates
)
...
async def _show_today_content(msg, cur: str, ctx, is_edit=True):
    """Fetch today's games from top leagues and display."""
    import asyncio
    from datetime import datetime as dt, timezone
    
    # Top leagues to check for today's games
    today_leagues = ["epl", "lal", "sea", "bun", "fl1", "ucl", "uel", "nba", "nhl", "atp", "mma", "cs2"]
    ...
    for lg, raw in zip(today_leagues, results):
        ...
        for ev in raw:
            ...
            if g.get("time"):
                try:
                    gt = dt.fromisoformat(g["time"].replace("Z", "+00:00"))
    ...
    for i, g in enumerate(all_games[:20]):
        ...
        lg_emoji = g.get("_league_emoji", "")
        
        if lg_name != current_league:
            current_league = lg_name
            text += f"{lg_emoji} <b>{lg_name}</b>\n"
        
        # Time
        match_time = ""
        if g.get("time"):
            try:
                t = dt.fromisoformat(g["time"].replace("Z", "+00:00"))
                match_time = t.strftime("%H:%M")
            except: pass
        
        home = g.get("home", "")[:12] or g.get("title", "")[:15]
        away = g.get("away", "")[:12]
        live_tag = "🔴" if g.get("live") else f"⏰{match_time}"
        
        if g["ml"]:
            h = fmt_odds(g["ml"]["home"])
            a = fmt_odds(g["ml"]["away"])
            text += f"  {live_tag} {home} vs {away}\n"
            text += f"  <code>{h:>6} │ {a:>5}</code>\n"
        else:
            text += f"  {live_tag} {home} vs {away}\n"
        ...
        # Button
        lg_key = g.get("_league", "today")
        btn = f"{live_tag} {g.get('home_s','?')} vs {g.get('away_s','?')}"
...
async def _show_league(q, league: str, cur: str, ctx):
    ...
    # Build text listing with dates + odds preview
    ...
    text += f"━━━━━━━━━━━━━━━━━━━━━━\n"
    if has_draw:
        text += f"<code>{'  1':>6} │ {'  X':>5} │ {'  2':>5}</code>\n\n"
    else:
        text += f"<code>{'  1':>6} │ {'  2':>5}</code>\n\n"
    ...
    for i, g in enumerate(games[:15]):
        ...
        if match_date and match_date != current_date:
            ...
            text += f"📅 <b>{match_date}</b>\n"
        
        # Match line with odds preview
        home = g.get("home", "")[:12] or g.get("title", "")[:12]
        away = g.get("away", "")[:12]
        ...
        if g["ml"]:
            ...
            if has_draw and ml.get("draw", 0) > 0:
                d = fmt_odds(ml["draw"])
                odds_str = f"<code>{h:>6} │ {d:>5} │ {a:>5}</code>"
            elif has_draw:
                odds_str = f"<code>{h:>6} │   —  │ {a:>5}</code>"
            else:
                odds_str = f"<code>{h:>6} │ {a:>5}</code>"
        else:
            if has_draw:
        ...
        live_tag = "🔴 " if g.get("live") else ""
        time_tag = f"⏰{match_time} " if match_time else ""
        
        text += f"{live_tag}{time_tag}<b>{home}</b> vs <b>{away}</b>\n"
        text += f"  {odds_str}\n\n"
        
        # Button
        btn_label = f"{g.get('home_s', 'HOM')} vs {g.get('away_s', 'AWY')}"
        if match_time:
            btn_label = f"⏰{match_time}  {btn_label}"
        if g.get("live"):
