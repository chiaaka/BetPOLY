...
from polymarket_api import (
    ...
    refresh_currency_rates, get_mid_price, fetch_buy_prices_for_game
)
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "main_menu":
        ...
        if idx < len(games):
            ...
            # Fetch actual BUY prices
            try:
                await fetch_buy_prices_for_game(g)
            except Exception as e:
                logger.warning(f"Could not fetch buy prices: {e}")
            info = LEAGUES.get(league, {"name": "Today", "emoji": "📅"})
            text = f"{info['emoji']} {info['name']}\n\n{format_match(g, cur)}"
            if g.get("_wide_spread"):
                text += "\n⚠️ <i>Low liquidity — actual fill price may differ from displayed odds</i>"
...
async def _show_game(q, league: str, idx: int, cur: str, ctx):
    ...
    # Fetch actual BUY prices from orderbook (what user will pay)
    try:
        await fetch_buy_prices_for_game(g)
    except Exception as e:
        logger.warning(f"Could not fetch buy prices: {e}")
    ...
    # Add spread warning for thin markets
    if g.get("_wide_spread"):
        text += "\n⚠️ <i>Low liquidity — actual fill price may differ from displayed odds</i>"
