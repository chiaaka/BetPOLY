...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Sell Position ---
        ...
        await q.edit_message_text(
            ...
            f"🔢 Shares: {shares:.2f}\n"
            f"💵 Avg Price: ${bet_row.get('price', 0):.2f}\n"
            ...
            if bets:
                ...
                for b in bets[:10]:
                    ...
                    if b.get("status") == "placed":
                        text += f"  Shares: {shares:.2f} | Max Win: {fmt_currency(max_win, cur)}\n"
                    text += f"  Status: {b.get('status', 'unknown')}\n\n"
                    # Show correct button based on status
                    if b.get("status") == "placed":
                        buttons.append([
                            Btn(f"📊 Sell #{b['id']}", callback_data=f"sell_{b['id']}"),
                        ])
                    elif b.get("status") == "won":
                        buttons.append([
                            Btn(f"🏆 Redeem #{b['id']}", callback_data=f"redeem_{b['id']}"),
                        ])
...
async def _check_settlements(context):
    """Background job: check if any active bets have resolved.
    Sends notification that auto-deletes after 3 minutes."""
    try:
        active_bets = await db.get_active_bets()
        if not active_bets:
            return
        ...
        for bet in active_bets:
            ...
            try:
                # Check if market is resolved by looking at market data
                from py_clob_client.client import ClobClient as _ClobCheck
                check_client = _ClobCheck("https://clob.polymarket.com", chain_id=137)
                
                # Try to get price — if market is closed/resolved, this may fail or return 0/1
                try:
                    price_data = check_client.get_price(token_id, side="BUY")
                    price = float(price_data.get("price", 0.5)) if isinstance(price_data, dict) else float(price_data)
                except:
                    price = -1  # Can't get price = might be resolved
                
                # Check via order book if market is still active
                try:
                    ob = check_client.get_order_book(token_id)
                    if not ob or (not ob.get("bids") and not ob.get("asks")):
                        # Empty order book = likely resolved
                        price = -1
                except:
                    pass
                
                if price == -1 or price >= 0.99 or price <= 0.01:
                    ...
                    if price >= 0.99:
                        # Won!
                        await db.update_bet_status(bet_id, "won", shares)
                        payout = round(shares * 1.0, 2)
                        msg = await context.bot.send_message(
                            ...
                            text=f"🏆 <b>You Won!</b>\n\n"
                                 ...
                                 f"✅ {selection}\n"
                                 f"💰 Payout: ${payout:.2f}\n\n"
                                 f"Tap Redeem to collect your winnings!",
                            parse_mode=ParseMode.HTML,
                            reply_markup=Markup([
                                [Btn("🏆 Redeem Now", callback_data=f"redeem_{bet_id}")],
                            ])
                        )
                        ...
                        # Lost
                        await db.update_bet_status(bet_id, "lost", 0)
                        msg = await context.bot.send_message(
                            chat_id=user_id,
                            text=f"😔 <b>Bet Lost</b>\n\n"
                                 f"📋 {match_name}\n"
                ...
                logger.debug(f"Settlement check error for bet {bet.get('id')}: {e}")
    except Exception as e:
