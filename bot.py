...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Refresh odds on confirmation screen ---
        ...
        if not bet or not bet.get("token_id"):
            ...
            return
        try:
            new_price = trade.get_current_price(bet["token_id"])
            if new_price and new_price > 0:
                ...
                await q.answer("⚠️ Could not fetch price")
        except Exception as e:
...
async def _show_confirm(q, stake: float, bet: dict, cur: str):
    # Fetch fresh price before showing confirmation
    try:
        token_id = bet.get("token_id", "")
        if token_id:
            fresh_price = trade.get_current_price(token_id)
            if fresh_price and fresh_price > 0:
                bet["price"] = fresh_price
                bet["odds"] = round(1.0 / fresh_price, 2) if fresh_price > 0 else 0
    except Exception as e:
        logger.warning(f"Could not refresh price for confirmation: {e}")
    
    price = bet.get("price", bet["odds"])
    odds = bet["odds"]  # Decimal odds (e.g. 1.91)
