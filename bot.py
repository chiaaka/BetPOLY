...
try:
    # Check BuilderConfig constructor
    from py_builder_signing_sdk.config import BuilderConfig
    import inspect
    print("=== BuilderConfig ===")
    print(inspect.signature(BuilderConfig.__init__))
    print(inspect.getsource(BuilderConfig.__init__)[:1500])
    
    # Check SafeTransaction model
    from py_builder_relayer_client.models import SafeTransaction
    print("=== SafeTransaction ===")
    print(inspect.getsource(SafeTransaction)[:1000])
    
    # Check RelayClient.deploy and execute signatures
    from py_builder_relayer_client.client import RelayClient
    print("=== RelayClient.deploy ===")
    print(inspect.getsource(RelayClient.deploy)[:1500])
    print("=== RelayClient.execute ===")
    print(inspect.getsource(RelayClient.execute)[:1500])
    
    # Check derive
    from py_builder_relayer_client.builder.derive import derive
    print("=== derive ===")
    print(inspect.getsource(derive)[:1000])
    
    # Check ClientRelayerTransactionResponse
    from py_builder_relayer_client.response import ClientRelayerTransactionResponse
    print("=== ClientRelayerTransactionResponse ===")
    print(inspect.getsource(ClientRelayerTransactionResponse)[:1500])
