...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
            ...
            try:
                stats = await asyncio.wait_for(
                    asyncio.get_event_loop().run_in_executor(
                        None, lambda: _sync_get_user_stats(addr)
                    ), timeout=10
                )
                return {
        ...
        # Fetch all concurrently (max 10 at a time to avoid hammering API)
        sem = asyncio.Semaphore(10)
        async def fetch_limited(r):
            async with sem:
                return await fetch_one(r)
        
        user_stats = await asyncio.gather(*[fetch_limited(r) for r in rows])
