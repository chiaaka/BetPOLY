...
def kb_game_odds(game: dict, idx: int, league: str):
    ...
    # Handicap / Spread — ALL lines
    if game.get("spreads"):
        for si, sp in enumerate(game["spreads"]):
            r = []
            if sp["home"] > 0:
                r.append(Btn(f"H1 ({sp['line']}) │ {fmt_odds(sp['home'])}", callback_data=f"b_{league}_{idx}_sp_{si}_h"))
            if sp["away"] > 0:
                r.append(Btn(f"H2 ({sp['line']}) │ {fmt_odds(sp['away'])}", callback_data=f"b_{league}_{idx}_sp_{si}_a"))
            if r:
                rows.append(r)
