...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(db._DB) as conn:
        ...
        # Get all safe addresses for volume calc
        rows = await conn.execute_fetchall("SELECT safe_address FROM users WHERE safe_address IS NOT NULL AND safe_address != ''")
    
    # Total volume across all users from API
    total_volume = 0
    for r in rows:
        try:
            stats = await trade.get_user_stats_from_api(r[0])
