...
def kb_game_odds(game: dict, idx: int, league: str):
    ...
    if game.get("set1"):
        ...
        r = []
        if s1["home"] > 0:
            r.append(Btn(f"S1: {game.get('home_s', 'P1')} │ {fmt_odds(s1['home'])}", callback_data=f"b_{league}_{idx}_s1_h"))
        if s1["away"] > 0:
            r.append(Btn(f"S1: {game.get('away_s', 'P2')} │ {fmt_odds(s1['away'])}", callback_data=f"b_{league}_{idx}_s1_a"))
        if r:
            rows.append(r)
