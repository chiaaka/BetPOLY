...
def kb_game_odds(game: dict, idx: int, league: str):
    """
    ⭐ FIXED: Display Moneyline first, then 1st Set Winner directly below (Tennis only)

    Tappable odds buttons — clean Sportybet layout for all sports.
    For Tennis: Shows Match Winner (Moneyline) and 1st Set Winner separately.
    """
    rows = []
    sport = game.get("sport", "soccer")

    # ═══════════════════════════════════════════════════════════════════════════════
    # MONEYLINE / WINNER (Main Market) — Shows first for all sports
    # ═══════════════════════════════════════════════════════════════════════════════
    if game.get("ml"):
        ml = game["ml"]
        r = []
    ...
    # ═══════════════════════════════════════════════════════════════════════════════
    # ⭐ 1ST SET WINNER (TENNIS ONLY) — Shows directly below moneyline
    # ═══════════════════════════════════════════════════════════════════════════════
    if sport == "tennis" and game.get("set1"):
        s1 = game["set1"]
        r = []
        if s1["home"] > 0:
            r.append(Btn(f"S1: {game.get('home_s', 'P1')} │ {fmt_odds(s1['home'])}",
                         callback_data=f"b_{league}_{idx}_s1_h"))
        if s1["away"] > 0:
            r.append(Btn(f"S1: {game.get('away_s', 'P2')} │ {fmt_odds(s1['away'])}",
                         callback_data=f"b_{league}_{idx}_s1_a"))
        if r:
            rows.append(r)
    ...
    if game.get("totals"):
        ...
        if main:
            ...
            r = []
            if main["over"] > 0:
                r.append(Btn(f"Ov {main['line']} │ {fmt_odds(main['over'])}",
                            callback_data=f"b_{league}_{idx}_t_{main_idx}_o"))
            if main["under"] > 0:
                r.append(Btn(f"Un {main['line']} │ {fmt_odds(main['under'])}",
                            callback_data=f"b_{league}_{idx}_t_{main_idx}_u"))
            if r:
                rows.append(r)
            ...
            for t in filtered:
                ...
                r = []
                if t["over"] > 0:
                    r.append(Btn(f"{ov_label} │ {fmt_odds(t['over'])}",
                                callback_data=f"b_{league}_{idx}_t_{ti}_o"))
                if t["under"] > 0:
                    r.append(Btn(f"{un_label} │ {fmt_odds(t['under'])}",
                                callback_data=f"b_{league}_{idx}_t_{ti}_u"))
                if r:
                    rows.append(r)
    ...
    if game.get("spreads"):
        ...
        for sp in filtered_sp:
            ...
            r = []
            if sp["home"] > 0:
                r.append(Btn(f"{sp_prefix}1 ({h_line}) │ {fmt_odds(sp['home'])}",
                            callback_data=f"b_{league}_{idx}_sp_{si}_h"))
            if sp["away"] > 0:
                r.append(Btn(f"{sp_prefix}2 ({a_line}) │ {fmt_odds(sp['away'])}",
                            callback_data=f"b_{league}_{idx}_sp_{si}_a"))
            if r:
                rows.append(r)
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Wallet ---
        ...
        if user:
            ...
            await q.edit_message_text(
                f"📥 <b>Deposit</b>\n\n"
                f"🔗 <b>Send Crypto (Any Chain)</b> ⭐ Recommended\n"
                f"Send USDC, ETH, BNB from any chain.\n"
                f"Auto-converts to USDC.e. Cheapest & fastest.\n\n"
                f"💳 <b>Buy with Card (MoonPay)</b>\n"
                f"Apple Pay, Google Pay, bank transfer.\n"
                f"First time requires quick ID check.\n\n"
                f"💰 <b>Send USDC.e (Polygon only)</b>\n"
                f"Direct deposit. Only USDC.e works here.\n"
                f"⚠️ Native USDC will NOT show up.\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"📬 Wallet: <code>{deposit_addr}</code>",
                parse_mode=ParseMode.HTML,
                reply_markup=Markup([
...
async def _handle_bet(q, data: str, cur: str, ctx):
    ...
    if mtype == "ml":
        ...
        s1 = g.get("set1", {})
        if side == "h":
            odds = price_to_odds(s1["home"])
            price = s1["home"]
            token_id = s1.get("home_tid", "")
            label = f"Set 1: {g['home_s']}"
        elif side == "a":
            odds = price_to_odds(s1["away"])
            price = s1["away"]
            token_id = s1.get("away_tid", "")
            label = f"Set 1: {g['away_s']}"
    ...
    logger.info(f"BET SELECT: {match_name} | {label} | token={token_id[:20]}... | price={price} | ml_home_tid={g.get('ml',{}).get('home_tid','')[:20]}")
    ctx.user_data["bet"] = {
