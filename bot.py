    ...
    # --- My Bets ---
        ...
        if user:
            ...
            if bets:
                ...
                rows = []
                for b in bets[:10]:
                    status_emoji = {"placed": "⏳", "won": "✅", "lost": "❌", "settled": "🏁", "sold": "💸"}.get(b.get("status", ""), "❓")
                    ...
                    # Add sell button for active bets
                    if b.get("status") == "placed" and b.get("token_id"):
                        bet_id = b.get("id")
                        rows.append([Btn(f"💸 Sell: {b.get('selection', '?')[:20]}", callback_data=f"sell_bet_{bet_id}")])
                rows.append([Btn("🔄 Refresh", callback_data="my_bets")])
                rows.append([Btn("🔙 Back", callback_data="main_menu")])
            else:
                text = "💰 <b>My Bets</b>\n\nNo bets yet. Pick a match to get started! 👇"
                rows = [[Btn("🔙 Back", callback_data="main_menu")]]
            await q.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=Markup(rows))
    
    # --- Sell Bet (show confirmation) ---
    elif d.startswith("sell_bet_"):
        bet_id = int(d.split("_")[2])
        bet = await db.get_bet_by_id(bet_id)
        if not bet or bet.get("telegram_id") != q.from_user.id:
            await q.answer("Bet not found.", show_alert=True)
            return
        if bet.get("status") != "placed":
            await q.answer("This bet is no longer active.", show_alert=True)
            return
        
        token_id = bet.get("token_id", "")
        shares = bet.get("shares", 0) or 0
        buy_price = bet.get("price", 0) or 0
        
        # Fetch current sell price
        try:
            from polymarket_api import fetch_token_price
            current_price = await fetch_token_price(token_id, side="SELL")
        except:
            current_price = 0
        
        sell_value = round(shares * current_price, 2) if current_price else 0
        buy_cost = bet.get("amount_net") or bet.get("amount_gross") or 0
        pnl = sell_value - buy_cost if sell_value else 0
        pnl_emoji = "📈" if pnl >= 0 else "📉"
        
        text = f"💸 <b>Sell Position</b>\n\n"
        text += f"<b>{bet.get('selection', '?')}</b>\n"
        text += f"{bet.get('match_name', '?')}\n\n"
        text += f"Bought: {shares:.2f} shares @ {buy_price:.3f}\n"
        text += f"Cost: ${buy_cost:.2f}\n\n"
        if current_price:
            text += f"Current price: {current_price:.3f}\n"
            text += f"Sell value: ${sell_value:.2f}\n"
            text += f"{pnl_emoji} P&L: ${pnl:+.2f}\n\n"
        else:
            text += "⚠️ Could not fetch current price\n\n"
        text += "Sell all shares now?"
        
        kb = Markup([
            [Btn(f"✅ Sell for ~${sell_value:.2f}", callback_data=f"sell_confirm_{bet_id}")],
            [Btn("🔙 Cancel", callback_data="my_bets")],
        ])
        await q.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb)
    
    # --- Sell Confirm (execute sell) ---
    elif d.startswith("sell_confirm_"):
        bet_id = int(d.split("_")[2])
        bet = await db.get_bet_by_id(bet_id)
        if not bet or bet.get("telegram_id") != q.from_user.id:
            await q.answer("Bet not found.", show_alert=True)
        ...
        try:
            ...
            if result.get("success"):
                ...
                await db.update_bet_status(bet_id, "sold", settled_amount=sold_amount)
                text = f"✅ <b>Position Sold!</b>\n\n"
                text += f"<b>{bet.get('selection', '?')}</b>\n"
                text += f"Shares sold: {bet.get('shares', 0):.2f}\n"
