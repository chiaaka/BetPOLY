            ...
            if safe_addr:
                try:
                    ...
                    if onchain_positions:
                        for i, p in enumerate(onchain_positions[:10]):
                            size = p.get("size", 0)
                            if size <= 0:
                                continue
                            ...
                            pnl_emoji = "📈" if pnl >= 0 else "📉"
                            status = "🏆 Redeemable" if redeemable else "⏳ Active"
                            
                            text += f"{'🏆' if redeemable else '⏳'} <b>{p.get('outcome', '?')}</b>\n"
                            text += f"  {p.get('title', '?')}\n"
                            text += f"  Shares: {size:.2f} @ ${p.get('avg_price', 0):.2f}\n"
                            text += f"  Value: ${current_val:.2f} {pnl_emoji} {'+' if pnl >= 0 else ''}{pnl_pct:.0f}%\n"
                            text += f"  {status}\n\n"
                            
                            token_id = p.get("token_id", "")
                            if redeemable and token_id:
                                buttons.append([
                                    Btn(f"🏆 Redeem", callback_data=f"redeem_onchain_{i}"),
                                ])
                            elif token_id:
                                buttons.append([
                                    Btn(f"📊 Sell", callback_data=f"sell_onchain_{i}"),
                                ])
                        
                        # Store positions for callback reference
                        ctx.user_data["onchain_positions"] = onchain_positions
                    else:
                        text += "No open positions found on-chain.\n\n"
                except Exception as e:
                    logger.warning(f"Could not fetch on-chain positions: {e}")
                    text += "⚠️ Could not load on-chain positions.\n\n"
                    # Fall back to DB
                    bets = await db.get_user_bets(q.from_user.id)
                    if bets:
                        for b in bets[:10]:
                            ...
                            text += f"  {b.get('match_name', '?')} | {fmt_currency(gross, cur)}\n\n"
            else:
                text += "No wallet set up yet. Use /start to create one.\n\n"
        ...
        for user_row in all_users:
            ...
            try:
                ...
                resp = _req.get(
                    ...
                    params={"user": safe_addr, "sizeThreshold": 0, "redeemable": True, "limit": 50},
                    timeout=10,
                )
                if not resp.ok:
                    continue
                ...
                for pos in positions:
                    ...
                    msg = await context.bot.send_message(
                        chat_id=user_id,
                        text=f"🏆 <b>Position Ready to Redeem!</b>\n\n"
                             f"📋 {title}\n"
                             f"✅ {outcome}\n"
                             f"🔢 Shares: {size:.2f}\n"
                             f"💰 Payout: ${payout:.2f}\n"
                             f"📈 P&L: {pnl_text}\n\n"
                             f"Tap My Bets → Redeem to collect!",
                        parse_mode=ParseMode.HTML,
                        reply_markup=Markup([
                            [Btn("📋 My Bets", callback_data="my_bets")],
                        ])
