        ...
        try:
            ...
            if book.get("bids"):
                ...
                if est["total_cost"] > 0:
                    ...
                    if not est["filled"]:
                        fill_warning = f"\n⚠️ <i>Not enough liquidity to sell all {shares:.2f} shares. Only ${est['depth_usdc']:.2f} available on buy side.</i>\n"
        except Exception as e:
...
async def _show_confirm(q, stake: float, bet: dict, cur: str):
            ...
            if book.get("asks"):
                ...
                if est["avg_price"] > 0 and est["filled"]:
                    ...
                    if displayed_odds > 0 and est_odds > 0:
                        diff_pct = abs(est_odds - displayed_odds) / displayed_odds * 100
                        if diff_pct > 15:
                            fill_warning = f"⚠️ <i>Estimated fill ({est_odds:.2f}) differs significantly from displayed odds ({displayed_odds:.2f}). Thin orderbook.</i>\n\n"
                elif not est["filled"]:
    ...
    if bet.get("verified_market"):
        verified_line = f"🔍 Market: <i>{esc(bet['verified_market'][:50])}</i>\n"
