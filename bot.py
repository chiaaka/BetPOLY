    ...
    # --- Wallet ---
        ...
        if user:
            ...
            await q.edit_message_text(
                ...
                parse_mode=ParseMode.HTML,
                reply_markup=Markup([
                    [Btn("💳 Buy with Card / Bank Transfer", callback_data="deposit_fiat")],
                    [Btn("💰 Send USDC (I have crypto)", callback_data="deposit_crypto")],
                    [Btn("🔙 Back", callback_data="wallet")],
                ])
            )
        ...
        if user:
            ...
            # Generate Transak URL with user's wallet pre-filled
            import urllib.parse
            ...
            # Detect user currency from their settings
            user_cur = user.get("currency", "USD")
            if user_cur == "NGN":
                transak_params["fiatCurrency"] = "NGN"
                transak_params["countryCode"] = "NG"
            elif user_cur == "KES":
                transak_params["fiatCurrency"] = "KES"
                transak_params["countryCode"] = "KE"
            elif user_cur == "GHS":
                transak_params["fiatCurrency"] = "GHS"
                transak_params["countryCode"] = "GH"
            elif user_cur == "ZAR":
                transak_params["fiatCurrency"] = "ZAR"
                transak_params["countryCode"] = "ZA"
            
            transak_url = f"https://global.transak.com/?{urllib.parse.urlencode(transak_params)}"
            
            if not TRANSAK_API_KEY:
                await q.edit_message_text(
                    "💳 <b>Buy Crypto</b>\n\n"
                    "⚠️ Fiat on-ramp is coming soon!\n\n"
                    "For now, you can buy USDC on exchanges like:\n"
                    "• Binance → Withdraw USDC on Polygon\n"
                    "• Luno → Buy and send to your address\n"
                    "• Bybit P2P → Buy USDC with NGN\n\n"
                    f"Your deposit address:\n<code>{deposit_addr}</code>",
                    parse_mode=ParseMode.HTML,
                    reply_markup=Markup([
                        [Btn("💰 Send USDC instead", callback_data="deposit_crypto")],
                        [Btn("🔙 Back", callback_data="w_deposit")],
                    ])
                )
            else:
                await q.edit_message_text(
                    "💳 <b>Buy USDC with Card / Bank Transfer</b>\n\n"
                    "Tap the button below to buy USDC.\n"
                    "It will be sent directly to your betting wallet.\n\n"
                    "✅ Supports NGN, KES, GHS, ZAR, USD, EUR\n"
                    "✅ Bank transfer, card, mobile money\n"
                    "✅ Arrives in ~5 minutes\n\n"
                    "<i>Powered by Transak — you'll complete payment on their secure page.</i>",
                    parse_mode=ParseMode.HTML,
                    reply_markup=Markup([
                        [Btn("💳 Buy USDC →", url=transak_url)],
                        [Btn("💰 Send USDC instead", callback_data="deposit_crypto")],
                        [Btn("🔙 Back", callback_data="w_deposit")],
                    ])
                )
        ...
        if user:
            ...
            await q.edit_message_text(
                f"💰 <b>Send USDC</b>\n\n"
                f"Send to this address on <b>Polygon network</b>:\n\n"
                f"<code>{deposit_addr}</code>\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"{setup_note}"
                f"✅ <b>Best option: USDC.e on Polygon</b>\n"
                f"Ready to bet immediately — no extra steps.\n\n"
                f"⚠️ <b>Sending USDC (not USDC.e) from your exchange?</b>\n"
                f"That works too! After it arrives:\n"
                f"Wallet → Balance → tap <b>Convert</b> (free, gasless)\n"
                f"Then you're ready to bet.\n\n"
                f"⚠️ <b>Only send on Polygon network.</b>\n"
                f"Sending on the wrong network may result in lost funds.\n\n"
                f"💰 Min bet: $1",
                parse_mode=ParseMode.HTML,
                reply_markup=Markup([
                    [Btn("💳 Buy with Card instead", callback_data="deposit_fiat")],
                    [Btn("🔙 Back", callback_data="w_deposit")],
                ])
            )
