...
from polymarket_api import (
    ...
    refresh_currency_rates, get_mid_price, fetch_orderbook, estimate_fill,
    _abbreviate
)
from live_scores import fetch_soccer_scores, fetch_nba_scores, fetch_nhl_scores, fetch_mma_scores, match_score_to_game, format_score_line, ESPN_SOCCER_LEAGUES
...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        users_week = await pool.fetchval("SELECT COUNT(*) FROM users WHERE created_at > $1", week_ago)
        
        # Users today
        day_ago = time.time() - 86400
        users_today = await pool.fetchval("SELECT COUNT(*) FROM users WHERE created_at > $1", day_ago)
        ...
        # Fetch per-user stats CONCURRENTLY with timeout
        sem = asyncio.Semaphore(10)
        
        async def fetch_one(r):
            async with sem:
                tg_id, addr, created = r["telegram_id"], r["safe_address"], r["created_at"]
                try:
                    stats = await asyncio.wait_for(
                        asyncio.get_event_loop().run_in_executor(
                            None, lambda a=addr: _sync_get_user_stats(a)
                        ), timeout=10
                    )
                    return {
                        ...
                        "open": stats.get("open_positions", 0),
                        "created": created,
                    }
                except:
                    return {
                        "tg_id": tg_id, "addr": addr,
                        "volume": 0, "profit": 0, "wins": 0,
                        "losses": 0, "open": 0, "created": created,
                    }
        
        user_stats = await asyncio.gather(*[fetch_one(r) for r in rows])
        user_stats = list(user_stats)
        
        total_volume = sum(u["volume"] for u in user_stats)
        total_profit = sum(u["profit"] for u in user_stats)
        ...
        await update.message.reply_text(f"❌ Admin error: {e}", parse_mode=ParseMode.HTML)


def _sync_get_user_stats(safe_address: str) -> dict:
    """Synchronous version for run_in_executor — fast with short timeout."""
    import requests
    stats = {"volume": 0, "wins": 0, "losses": 0, "profit": 0, "open_positions": 0}
    try:
        resp = requests.get(
            "https://data-api.polymarket.com/activity",
            params={"user": safe_address, "type": "TRADE", "side": "BUY", "limit": 500},
            timeout=8,
        )
        if resp.ok:
            for t in (resp.json() or []):
                usdc_size = t.get("usdcSize")
                if usdc_size and float(usdc_size) > 0:
    ...
    try:
        resp = requests.get(
            ...
            params={"user": safe_address, "limit": 100},
            timeout=8,
        )
        if resp.ok:
    ...
    try:
        resp = requests.get(
            ...
            params={"user": safe_address, "sizeThreshold": 0, "limit": 100},
            timeout=8,
        )
        if resp.ok:
            stats["open_positions"] = len(resp.json() or [])
    except:
        pass
    return stats
...
async def cmd_users_export(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        addr_list = []
        for r in rows:
            ts = time.strftime("%Y-%m-%d", time.gmtime(r["created_at"])) if r["created_at"] else "?"
            lines.append(f"{r['telegram_id']},{r['safe_address']},{ts}")
            addr_list.append(r["safe_address"])
        ...
        await update.message.reply_text(f"❌ Export error: {e}", parse_mode=ParseMode.HTML)
...
async def cmd_broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Admin: broadcast a message to all users. Usage: /broadcast <message>"""
    if update.effective_user.id != ADMIN_TELEGRAM_ID:
        await update.message.reply_text("⛔ Unauthorized.")
        return
    ...
    try:
        ...
        await update.message.reply_text(
            f"📢 Broadcasting to {total} users...", parse_mode=ParseMode.HTML
        )
...
async def cmd_broadcast_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Admin: broadcast a photo with caption. Reply to a photo with /broadcastphoto <caption>"""
    if update.effective_user.id != ADMIN_TELEGRAM_ID:
        await update.message.reply_text("⛔ Unauthorized.")
        return
    ...
    if not reply or not reply.photo:
        await update.message.reply_text(
            "📸 <b>Photo Broadcast Usage:</b>\n\n"
            "1. Send the image you want to broadcast\n"
            "2. Reply to that image with:\n"
            "<code>/broadcastphoto Your caption here</code>\n\n"
            "Supports HTML formatting in caption.",
            parse_mode=ParseMode.HTML
        )
        return
    
    photo = reply.photo[-1].file_id  # Highest resolution
    raw_cap = update.message.text or ""
    caption = raw_cap.split(None, 1)[1] if len(raw_cap.split(None, 1)) > 1 else ""
    ...
    try:
        ...
        await update.message.reply_text(
            f"📢 Broadcasting photo to {total} users...", parse_mode=ParseMode.HTML
        )
...
async def cmd_bettors(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Admin: list all users who have placed at least one bet with stats."""
    if update.effective_user.id != ADMIN_TELEGRAM_ID:
        await update.message.reply_text("⛔ Unauthorized.")
        return
    ...
    try:
        ...
        rows = await pool.fetch("""
            ...
            FROM users u
            INNER JOIN bets b ON u.telegram_id = b.telegram_id
            GROUP BY u.telegram_id, u.username, u.safe_address, u.total_volume, u.total_fees_paid
        ...
        for i, r in enumerate(rows, 1):
            ...
            addr = r["safe_address"][:6] + "..." + r["safe_address"][-4:] if r["safe_address"] else "N/A"
            vol = r["total_volume"] or 0
            bets = r["bet_count"]
            last = ""
            if r["last_bet"]:
                ago = time.time() - r["last_bet"]
                if ago < 3600:
                    last = f"{int(ago/60)}m ago"
                elif ago < 86400:
                    last = f"{int(ago/3600)}h ago"
                else:
                    last = f"{int(ago/86400)}d ago"
            
            lines.append(
                f"{i}. @{username} | {addr}\n"
                f"   Vol: ${vol:.2f} | Bets: {bets} | Last: {last}"
            )
        
        header = (
            f"🎰 <b>Active Bettors ({len(rows)})</b>\n\n"
        )
        
        body = "\n".join(lines)
        
        # Split if too long
        text = header + f"<pre>{body}</pre>"
        if len(text) > 4000:
            # Send in chunks
            ...
            for line in lines:
                if len(chunk) + len(line) > 3800:
                    await update.message.reply_text(f"<pre>{chunk}</pre>", parse_mode=ParseMode.HTML)
                    chunk = ""
                chunk += line + "\n"
            if chunk:
                await update.message.reply_text(f"<pre>{chunk}</pre>", parse_mode=ParseMode.HTML)
        ...
        # Summary
        total_vol = sum(r["total_volume"] or 0 for r in rows)
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "main_menu":
        ...
        if idx < len(games):
            ...
            score_line = ""
            try:
                sport = g.get("sport", g.get("_sport", ""))
                if sport == "soccer":
                    scores = await fetch_soccer_scores(league)
                    match = match_score_to_game(g, scores) if scores else None
                    if not match:
                        # Today games have _league set, so this should work
                        import asyncio
                        from live_scores import ESPN_SOCCER_LEAGUES
                        all_leagues = list(ESPN_SOCCER_LEAGUES.keys())
                        tasks = [fetch_soccer_scores(lg) for lg in all_leagues]
                        results = await asyncio.gather(*tasks, return_exceptions=True)
                        for sc in results:
                            if isinstance(sc, Exception) or not sc:
                                continue
                            match = match_score_to_game(g, sc)
                            if match:
                                break
                    if match and match.get("state") in ("in", "post"):
                        score_line = format_score_line(match, "soccer")
                elif sport == "basketball" or league == "nba":
                    scores = await fetch_nba_scores()
                    match = match_score_to_game(g, scores)
                    if match and match.get("state") in ("in", "post"):
                        score_line = format_score_line(match, "basketball")
                elif sport == "hockey" or league == "nhl":
                    scores = await fetch_nhl_scores()
                    match = match_score_to_game(g, scores)
                    if match and match.get("state") in ("in", "post"):
                        score_line = format_score_line(match, "hockey")
                elif sport == "mma" or league == "mma":
                    scores = await fetch_mma_scores()
                    match = match_score_to_game(g, scores)
                    if match and match.get("state") in ("in", "post"):
                        score_line = format_score_line(match, "mma")
            except Exception:
                pass
    ...
    # --- Wallet ---
        ...
        if user:
            await q.edit_message_text(
                ...
                "💳  <b>Buy with Card</b> — MoonPay\n"
                "     Visa, Mastercard, Apple Pay, bank transfer\n\n"
                "🔗  <b>Send Crypto</b> — Any chain, any token\n"
                "     Auto-converts to betting balance\n\n"
                "💰  <b>Direct Transfer</b> — USDC.e on Polygon\n"
                "     For advanced users only",
                parse_mode=ParseMode.HTML,
                reply_markup=Markup([
                    [Btn("💳 Buy with Card", callback_data="deposit_moonpay")],
                    [Btn("🔗 Send Crypto (Any Chain) ⭐", callback_data="deposit_bridge")],
                    [Btn("💰 Direct USDC.e (Polygon)", callback_data="deposit_crypto")],
        ...
        if user:
            ...
            await q.edit_message_text(
                "💳 <b>Buy with Card — MoonPay</b>\n\n"
                
                "━━━━━━━  How it works  ━━━━━━━\n\n"
                
                "1️⃣  Tap <b>\"Open MoonPay\"</b> below\n"
                "2️⃣  Select your currency & amount\n"
                "3️⃣  Choose payment method\n"
                "      Card · Apple Pay · Bank · Google Pay\n"
                "4️⃣  Verify ID (first time only, ~2 min)\n"
                "5️⃣  Confirm — USDC arrives in ~5 min\n\n"
                
                "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                
                "🌍 <b>Region not supported?</b>\n"
                "Use <b>Send Crypto</b> instead:\n"
                "Buy USDC on Binance → send via BSC →\n"
                "auto-converts to your betting balance.",
                parse_mode=ParseMode.HTML,
                reply_markup=Markup([
                    [Btn("🌙 Open MoonPay →", url=moonpay_url)],
                    [Btn("🔗 Send Crypto instead", callback_data="deposit_bridge")],
        ...
        if user:
            ...
            if bridge and bridge.get("evm"):
                ...
                await q.edit_message_text(
                    ...
                    "Send any supported token to the address\n"
                    "below. It auto-converts to your betting balance.\n\n"
                    
                    "━━━━━  EVM Deposit Address  ━━━━━\n"
                    "<i>Works for BSC, Base, Arbitrum, Optimism,\n"
                    "Polygon, Ethereum — one address for all</i>\n\n"
                    
                    f"<code>{evm_addr}</code>\n\n"
                    
                    "  <b>BSC</b>  ·  BNB · USDC · USDT · BUSD\n"
                    "  <b>Base</b>  ·  ETH · USDC · USDT\n"
                    "  <b>Arbitrum</b>  ·  ETH · USDC · ARB\n"
                    "  <b>Optimism</b>  ·  ETH · USDC · OP\n"
                    "  <b>Polygon</b>  ·  USDC · USDC.e · POL\n"
                    "  <b>Ethereum</b>  ·  ETH · USDC · USDT\n"
                    
                    f"{extra_chains}\n"
                    
                    "━━━━━━  ⚠️ Important  ━━━━━━━\n\n"
                    
                    "🔸 <b>Minimum deposit amounts:</b>\n"
                    "     BSC / Base / Arb / OP / Polygon  →  <b>$2</b>\n"
                    "     Ethereum  →  <b>$7</b>\n"
                    "     Bitcoin / Tron  →  <b>$9</b>\n\n"
                    
                    "🔸 <b>Below minimum = funds lost.</b>\n"
                    "     Always send above the minimum!\n\n"
                    
                    "🔸 <b>Only send supported tokens.</b>\n"
                    "     Unsupported tokens = lost funds.\n\n"
                    
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                    
                    "💡 <b>Quick guide (Binance users):</b>\n"
                    "Binance → Withdraw → USDC → Network: BSC\n"
                    "(BEP20) → Paste address above → Confirm\n"
                    "Arrives in ~2 min. Gas: under $0.01\n\n"
                    
                    "⏱ All deposits arrive in 2–10 minutes.",
                    parse_mode=ParseMode.HTML,
                    reply_markup=Markup([
                ...
                await q.edit_message_text(
                    ...
                    "❌ Couldn't load multi-chain addresses.\n\n"
                    
                    "You can still send <b>USDC.e</b> directly\n"
                    "on Polygon to your wallet:\n\n"
        ...
        if user:
            ...
            await q.edit_message_text(
                "💰 <b>Direct Transfer — USDC.e on Polygon</b>\n\n"
                ...
                "✅  <b>USDC.e (Bridged USDC)</b> on Polygon\n"
                "     Ready to bet immediately.\n\n"
                
                "⚠️  <b>Native USDC</b> on Polygon\n"
                "     Will appear in wallet but requires\n"
                "     conversion before betting.\n"
                "     (Wallet → Balance → Convert button)\n\n"
                
                "❌  <b>Any other chain</b> = funds lost!\n"
                "     BSC, Base, Arbitrum, etc. will NOT\n"
                "     arrive. Use \"Send Crypto\" option\n"
                "     for other chains.\n\n"
                
                "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                
                "💡 <b>Recommended:</b> Use \"Send Crypto (Any\n"
                "Chain)\" — auto-converts everything for you.",
                parse_mode=ParseMode.HTML,
                reply_markup=Markup([
                    [Btn("🔗 Send Crypto (Any Chain) ⭐", callback_data="deposit_bridge")],
                    [Btn("💳 Buy with Card", callback_data="deposit_moonpay")],
    ...
    # --- Sell Position (from DB) ---
        ...
        # Fetch orderbook for real sell estimate
        fill_warning = ""
        slippage_warning = ""
        try:
            book = await fetch_orderbook(token_id)
            if book.get("bids"):
                est = estimate_fill(book, "SELL", shares)
                if est["total_cost"] > 0:
                    sell_value = round(est["total_cost"], 2)
                    if est["avg_price"] > 0:
                        slippage_pct = 0
                        if current_price > 0 and est["avg_price"] > 0:
                            slippage_pct = ((current_price - est["avg_price"]) / current_price) * 100
                        if slippage_pct > 3:
                            slippage_warning = f"\n⚠️ <i>Sell slippage: {slippage_pct:.1f}% — you'll get ${est['avg_price']:.2f}/share instead of ${current_price:.2f}</i>"
                        current_price = est["avg_price"]
                    if not est["filled"]:
                        fill_warning = f"\n⚠️ <i>Not enough buyers for all {shares:.2f} shares. Only ${est['depth_usdc']:.2f} available on buy side.</i>\n"
        except Exception as e:
            logger.debug(f"Sell fill estimation failed: {e}")
...
async def _show_league(q, league: str, cur: str, ctx, page: int = 0):
    ...
    # Cache expires after 3 minutes to pick up fresh data
    import time as _time
    cache_key = f"g_{league}"
    cache_ts_key = f"g_{league}_ts"
    games = ctx.user_data.get(cache_key)
    cache_ts = ctx.user_data.get(cache_ts_key, 0)
    cache_age = _time.time() - cache_ts
    
    if games is None or cache_age > 180:  # 3 minute expiry
        games = None  # Force refresh
    ...
    if games is None:
        ...
        games = [parse_event(g) for g in raw]
        ctx.user_data[f"g_{league}"] = games
        ctx.user_data[f"g_{league}_ts"] = _time.time()
...
async def _show_game(q, league: str, idx: int, cur: str, ctx):
    ...
    try:
        sport = g.get("sport", "")
        actual_league = g.get("_league", league)
        
        if sport == "soccer":
            match = None
            # Try specific league if known
            if actual_league in {"epl", "lal", "sea", "bun", "fl1", "ucl", "uel", "uecl", "mls", "erd", "lig", "spl", "bel", "sup"}:
                scores = await fetch_soccer_scores(actual_league)
                if scores:
                    match = match_score_to_game(g, scores)
            
            # If no match yet, try all major leagues in parallel
            if not match:
                import asyncio
                from live_scores import ESPN_SOCCER_LEAGUES
                all_leagues = list(ESPN_SOCCER_LEAGUES.keys())
        ...
        elif sport == "basketball" or actual_league == "nba" or league == "nba":
            scores = await fetch_nba_scores()
            match = match_score_to_game(g, scores)
            if match and match.get("state") in ("in", "post"):
                score_line = format_score_line(match, "basketball")
        elif sport == "hockey" or actual_league == "nhl" or league == "nhl":
            scores = await fetch_nhl_scores()
            match = match_score_to_game(g, scores)
            if match and match.get("state") in ("in", "post"):
                score_line = format_score_line(match, "hockey")
        elif sport == "mma" or actual_league == "mma" or league == "mma":
            scores = await fetch_mma_scores()
            match = match_score_to_game(g, scores)
            if match and match.get("state") in ("in", "post"):
                score_line = format_score_line(match, "mma")
    except Exception as e:
        logger.debug(f"Live score fetch failed: {e}")
...
async def _handle_bet(q, data: str, cur: str, ctx):
    ...
    if mtype == "ml":
        ...
        if side == "h":
            ...
            # Use the actual outcome name from the API, not parsed home
            label = f"1 ({ml.get('home_name') or g['home']})"
        elif side == "d":
            ...
            # Use the actual outcome name from the API, not parsed away
            label = f"2 ({ml.get('away_name') or g['away']})"
        ...
        if side == "h":
            ...
            sp_name = _abbreviate(sp.get("home_name", "")) if sp.get("home_name") else g.get('home_s', '?')
            label = f"H1 ({sp_name} {sp.get('home_line', sp['line'])})"
        else:
            odds = price_to_odds(sp["away"])
            price = sp["away"]
            token_id = sp.get("away_tid", "")
            sp_name = _abbreviate(sp.get("away_name", "")) if sp.get("away_name") else g.get('away_s', '?')
            label = f"H2 ({sp_name} {sp.get('away_line', '+' + str(sp.get('line_num', '')))})"
...
async def _handle_stake(q, data: str, cur: str, ctx):
    ...
    if data == "stake_custom":
        await q.edit_message_text(
            f"🎫 <b>{esc(bet['label'])}</b> @ <b>{bet['odds']:.2f}</b>\n\n"
...
async def _show_confirm(q, stake: float, bet: dict, cur: str):
    ...
    # ── SAFETY CHECK: Verify token matches what user thinks they're betting on ──
    token_id = bet.get("token_id", "")
    market_name = ""  # What the market is actually called on Polymarket
    safety_warning = ""
    try:
        if token_id:
            from polymarket_api import verify_token_market
            verification = await verify_token_market(token_id)
            if verification:
                market_name = verification.get("question", "") or verification.get("market_name", "")
                outcome_name = verification.get("outcome", "")
                market_lower = market_name.lower()
                
                # Check 1: Is this a player prop when user expects moneyline/totals/spread?
                prop_keywords = ["score", "scorer", "goal", "card", "booking", "assist",
                                 "shot", "foul", "corner", "penalty", "clean sheet",
                                 "hat trick", "anytime", "mvp", "man of the match",
                                 "player", "prop"]
                is_prop = any(k in market_lower for k in prop_keywords)
                
                bet_label_lower = bet.get("label", "").lower()
                is_ml_bet = bet_label_lower.startswith("1 ") or bet_label_lower.startswith("2 ") or "draw" in bet_label_lower
                is_total_bet = "over" in bet_label_lower or "under" in bet_label_lower
                is_spread_bet = bet_label_lower.startswith("h1") or bet_label_lower.startswith("h2")
                
                should_block = False
                block_reason = ""
                
                if is_prop and (is_ml_bet or is_total_bet or is_spread_bet):
                    should_block = True
                    block_reason = "Player prop market detected instead of match market"
                
                # Check 2: For moneyline bets, verify team name is in the market question
                if is_ml_bet and not should_block:
                    # Extract team name from label like "1 (Leeds United)" or "2 (Man City)"
                    import re as _re
                    team_match = _re.search(r'\((.+?)\)', bet.get("label", ""))
                    if team_match:
                        team_name = team_match.group(1).lower()
                        # Check if any significant word from team name appears in market
                        team_words = [w for w in team_name.split() if len(w) > 2 and w not in ("fc", "the", "city", "united")]
                        if team_words and not any(w in market_lower for w in team_words):
                            should_block = True
                            block_reason = f"Team '{team_match.group(1)}' not found in market question"
                
                if should_block:
                    logger.error(f"  ❌ TOKEN MISMATCH: label='{bet.get('label')}' market='{market_name}' reason='{block_reason}'")
                    await q.edit_message_text(
                        "🚫 <b>BET BLOCKED — Safety Check Failed</b>\n\n"
                        f"You selected: <b>{esc(bet.get('label', ''))}</b>\n"
                        f"But the market is actually:\n"
                        f"<b>{esc(market_name)}</b>\n\n"
                        f"Reason: {esc(block_reason)}\n\n"
                        "The bet was <b>NOT placed</b> to protect your funds.\n\n"
                        "💡 Go back and reload the match — the data\n"
                        "should be correct now.",
                        parse_mode=ParseMode.HTML,
                        reply_markup=Markup([
                            [Btn("🔙 Back to Matches", callback_data="main_menu")]
                        ])
                    )
                    return
                
                # Show what they're actually betting on in the confirm
                if market_name and outcome_name:
                    bet["verified_market"] = market_name
                    bet["verified_outcome"] = outcome_name
    except Exception as e:
        logger.warning(f"Token verification failed (non-fatal): {e}")
    
    # Fetch orderbook and estimate actual fill
    fill_warning = ""
    try:
        from polymarket_api import fetch_orderbook, estimate_fill
        if token_id:
            book = await fetch_orderbook(token_id)
            if book.get("asks"):
                est = estimate_fill(book, "BUY", stake)
                if est["avg_price"] > 0 and est["filled"]:
                    est_odds = est["avg_odds"]
                    est_shares = est["total_shares"]
                    est_max_win = round(est_shares * 1.0, 2)
                    est_profit = round(est_max_win - stake, 2)
                    est_roi = round((est_profit / stake) * 100, 1) if stake > 0 else 0
                    
                    # Replace displayed values with estimated fill
                    odds = est_odds
                    shares = round(est_shares, 2)
                    max_winnings = est_max_win
                    profit = est_profit
                    roi = est_roi
                    
                    # Warn if estimated fill is significantly worse than displayed
                    displayed_odds = bet.get("odds", 0) or (round(1.0 / bet.get("price", 0.5), 2) if bet.get("price", 0) > 0 else 0)
                    if displayed_odds > 0 and est_odds > 0:
                        slippage_pct = ((displayed_odds - est_odds) / displayed_odds) * 100
                        if slippage_pct > 3:
                            fill_warning = f"⚠️ <i>Slippage: {slippage_pct:.1f}% (displayed {displayed_odds:.2f} → fill {est_odds:.2f})</i>\n\n"
                elif not est["filled"]:
                    fill_warning = f"⚠️ <i>Not enough liquidity to fill ${stake:.2f}. Only ${est['depth_usdc']:.2f} available.</i>\n\n"
    except Exception as e:
        logger.debug(f"Fill estimation failed: {e}")
    
    # Show verified market info if available
    verified_line = ""
    if bet.get("verified_market"):
        verified_line = f"🔍 <i>{esc(bet['verified_market'])}</i>\n"
        if bet.get("verified_outcome"):
            verified_line += f"🎯 Outcome: <i>{esc(bet['verified_outcome'])}</i>\n"
...
def main():
    ...
    app.add_handler(CommandHandler("broadcastphoto", cmd_broadcast_photo))
    app.add_handler(CommandHandler("bettors", cmd_bettors))
