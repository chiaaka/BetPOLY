...
from polymarket_api import (
    LEAGUES, CURRENCIES, fetch_games, fetch_live, parse_game,
...
def kb_main():
    return Markup([
        ...
        [Btn("🥊 UFC", callback_data="league_ufc")],
...
def kb_soccer():
    leagues = [(k, v) for k, v in LEAGUES.items() if v["sport"] == "soccer"]
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if existing:
        await update.message.reply_text(
            f"Welcome back to <b>BetPoly</b> ⚽🔥\n\nPick a sport 👇",
            parse_mode=ParseMode.HTML, reply_markup=kb_main()
        )
        return ConversationHandler.END
    
    await update.message.reply_text(
        f"Welcome to <b>BetPoly</b> ⚽🔥\n\n"
        f"The easiest way to bet on sports.\n"
        f"Real odds. Instant payouts. No middleman.\n\n"
        f"🔐 <b>Set a 6-digit PIN</b> to create your wallet.\n"
        f"Don't forget it!\n\n"
        f"Type your PIN now:",
...
async def pin_set(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not pin.isdigit() or len(pin) != 6:
        await update.effective_chat.send_message("❌ Must be exactly 6 digits. Try again:")
        return PIN_SET
    
    ctx.user_data["pin"] = pin
    await update.effective_chat.send_message("👍 Now <b>confirm your PIN</b> — type it again:", parse_mode=ParseMode.HTML)
    return PIN_CONFIRM
...
async def pin_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if pin != ctx.user_data.get("pin"):
        ...
        await update.effective_chat.send_message("❌ PINs don't match. Start over.\n\nType your 6-digit PIN:")
        return PIN_SET
    ...
    key_msg = await update.effective_chat.send_message(
        ...
        f"📬 Address:\n<code>{w['address']}</code>\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"⚠️ <b>SAVE YOUR RECOVERY KEY</b>\n"
        f"This message deletes in 60 seconds!\n\n"
        f"🔑 Private Key:\n<code>{w['private_key']}</code>\n\n"
        f"Import into MetaMask to backup your wallet.\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━",
        parse_mode=ParseMode.HTML
    )
    ...
    await update.effective_chat.send_message(
        f"🎉 You're all set!\n\n"
        f"<b>How to bet:</b>\n"
        f"1. Send USDC (Polygon) to your address\n"
        f"2. Pick a match\n"
        f"3. Tap the odds\n\n"
        f"Let's go! 👇",
        parse_mode=ParseMode.HTML, reply_markup=kb_main()
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Navigation ---
    if d == "main_menu":
        await q.edit_message_text("⚽ <b>BetPoly</b> — Pick a sport 👇",
                                  parse_mode=ParseMode.HTML, reply_markup=kb_main())
