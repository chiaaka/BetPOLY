...
"""
# === TEMPORARY INSPECT - remove after checking logs ===
import logging as _lg
_lg.basicConfig(level=_lg.INFO)
_insp_log = _lg.getLogger("INSPECT")
try:
    import inspect as _insp
    
    from py_builder_signing_sdk.config import BuilderConfig
    _insp_log.info("=== BuilderConfig.__init__ ===")
    _insp_log.info(_insp.getsource(BuilderConfig.__init__)[:2000])
    
    from py_builder_relayer_client.models import SafeTransaction
    _insp_log.info("=== SafeTransaction ===")
    _insp_log.info(_insp.getsource(SafeTransaction)[:1500])
    
    from py_builder_relayer_client.client import RelayClient
    _insp_log.info("=== RelayClient.deploy ===")
    _insp_log.info(_insp.getsource(RelayClient.deploy)[:2000])
    _insp_log.info("=== RelayClient.execute ===")
    _insp_log.info(_insp.getsource(RelayClient.execute)[:2000])
    
    from py_builder_relayer_client.builder.derive import derive
    _insp_log.info("=== derive ===")
    _insp_log.info(_insp.getsource(derive)[:1500])
    
    from py_builder_relayer_client.response import ClientRelayerTransactionResponse
    _insp_log.info("=== ClientRelayerTransactionResponse ===")
    _insp_log.info(_insp.getsource(ClientRelayerTransactionResponse)[:1500])
except Exception as e:
    _insp_log.error("INSPECT ERROR: %s", e, exc_info=True)
# === END TEMPORARY INSPECT ===
import os
import sys
import logging
import asyncio
...
async def pin_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if ob["success"]:
            ...
            # Onboarding failed — store what we can, retry on first bet
            logger.error(f"Onboarding failed for {user.id}: {ob.get('error')}")
            safe_addr = wallet.derive_safe_address(w["private_key"])
