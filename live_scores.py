...
ESPN_SOCCER_LEAGUES = {
    ...
    # Spain
    "lal": "esp.1",
    "esp_cup": "esp.copa_del_rey",
    # Italy
    "sea": "ita.1",
    "ita_cup": "ita.coppa_italia",
    # Germany
    "bun": "ger.1",
    "bun2": "ger.2",
    "ger_cup": "ger.dfb_pokal",
    # France
    "fl1": "fra.1",
    "fl2": "fra.2",
    "fr_cup": "fra.coupe_de_france",
    # UEFA
    "ucl": "uefa.champions",
    "uel": "uefa.europa",
    ...
    "ligmx": "mex.1",
    "bra1": "bra.1",
    "copa_lib": "conmebol.libertadores",
...
def match_score_to_game(game: dict, scores: list) -> dict | None:
    ...
    logger.info(f"  Matching: home='{home}' away='{away}' title='{title}' against {len(scores)} ESPN scores")
...
def format_score_line(score: dict, sport: str = "soccer") -> str:
    ...
    if state == "in":
        ...
        if sport == "basketball":
            ...
            return f"🏁 FINAL\n🥊 {hs} - {as_}"
        elif "final" in detail.lower() or "ft" in detail.lower():
            return f"🏁 FULL TIME\n⚽ {hs} - {as_}"
        else:
            return f"🏁 {detail}\n⚽ {hs} - {as_}"
...
async def fetch_nhl_scores() -> list:
    """Fetch live/today NHL scores from ESPN."""
    cache_key = "nhl"
    cached = _score_cache.get(cache_key)
    if cached and (time.time() - cached[0]) < CACHE_TTL:
        return cached[1]
    ...
    try:
        ...
        logger.debug(f"ESPN NHL scores error: {e}")
        return []


async def fetch_mma_scores() -> list:
    """Fetch live/today UFC/MMA scores from ESPN."""
    cache_key = "mma"
    cached = _score_cache.get(cache_key)
    if cached and (time.time() - cached[0]) < CACHE_TTL:
        return cached[1]
    ...
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            ...
            for ev in data.get("events", []):
                ...
                home = away = None
                for c in competitors:
                    if c.get("homeAway") == "home" or c.get("order") == 1:
                        home = c
                    else:
                        away = c
                if not home or not away:
                    continue
                scores.append({
                    "home": home.get("athlete", {}).get("displayName", home.get("team", {}).get("displayName", "")),
                    "home_short": home.get("athlete", {}).get("shortName", home.get("team", {}).get("shortDisplayName", "")),
                    "away": away.get("athlete", {}).get("displayName", away.get("team", {}).get("displayName", "")),
                    "away_short": away.get("athlete", {}).get("shortName", away.get("team", {}).get("shortDisplayName", "")),
                    "home_score": int(home.get("score", "0") or "0"),
                    "away_score": int(away.get("score", "0") or "0"),
                    "state": status_type.get("state", ""),
                    "detail": status_type.get("detail", ""),
        ...
        logger.debug(f"ESPN MMA scores error: {e}")
        return []
